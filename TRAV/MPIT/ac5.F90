...
!       call MPI_SEND(lt,1, NDM_MPI_LOGICAL, 1, 10006, MPI_COMM_WORLD, status, ierr)
    ...
    elseif (rang==1) then
       call MPI_RECV(x,1, NDM_MPI_REAL_DOUBLE,1 ,10002,MPI_COMM_WORLD,status,ierr)
       call MPI_RECV(i,1, MPI_INTEGER, 1, 10005, MPI_COMM_WORLD,status, ierr)
!       call MPI_RECV(lt,1, NDM_MPI_LOGICAL, 1, 10006, MPI_COMM_WORLD, ierr)
       call MPI_RECV(lt,1, MPI_LOGICAL, 1, 10006, MPI_COMM_WORLD,  status,ierr)
    end if
      write(6,*)rang,x,i,lt
