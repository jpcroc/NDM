...
subroutine read_parameters()
  ...
  !idum = -1
  !write(*,*) 'WE SET IDUM=-1, SO DETERMINISTIC ALEA'
...
!  endif
  ...
  call getenv('CSADDLE', temporary)
  if (temporary .eq. '') then
     CSADDLE   = 'countsaddle.dat'
  else
     read(temporary,*) CSADDLE
  endif
  ...
  ! Info regarding initial displacement
  call getenv('Max_Step_Uphill', temporary)
  if (temporary .eq. '') then
     MAXSTEPUPHILL = 0.08             ! Size of initial displacement in Ang.
  else
     read(temporary,*) MAXSTEPUPHILL
  endif
  ...
  write(*,*)   '***********************************'
  write(*,*)   '******* NVECTOR_LANCZOS ISSSSS ****', NVECTOR_LANCZOS
  write(*,*)   '***********************************'
  ...
  allocate(selected_atoms(natoms))
  ...
  allocate(old_before_sc_projection(VECSIZE))
...
!  write(flog,'(A39,I16  )')  ' Maximum number of neighbours         : ', maxnei    
  ...
  write(flog,'(1X,A39,F12.4)') ' - Max Step Uphill                   : ', MAXSTEPUPHILL
