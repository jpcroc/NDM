...
! on va chercher a remonter vers le saddle point
  ...
  do iter = iter_init, MAXITER
    ...
    if (iter.eq.1) MAXIPERP=MAXIPERP_ORIG 
...
!	   if ( dabs(deplasare) > INCREMENT ) deplasare =  INCREMENT * sign(one_art,fpar) !/ sqrt(1.0d0*iter)
	   if ( dabs(deplasare) > MAXSTEPUPHILL )  then
            !deplasare =  INCREMENT * sign(one_art,fpar) / sqrt(1.0d0*iter)	   
	     deplasare =  MAXSTEPUPHILL * sign(one_art,fpar)! / sqrt(1.0d0*iter)
	    !deplasare =  0.04*sign(one_art,fpar) 
	     coucou=' '
    	     moucou='*'
          end if
!debug	   write(*,*) '*deplasare: ', deplasare,  'normal: ', sign(one_art,fpar) * dabs(fpar / eigenvalue) , 'forced: ',INCREMENT * sign(one_art,fpar)
       ...
       write(*,"('D','iter: ',i4,'  iper: ',i4,'  ener: ',d14.8,'  fpar: ',f10.4,'  fperp: ',f10.4,&
