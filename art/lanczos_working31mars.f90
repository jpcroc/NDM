...
subroutine lanczos(maxvec,new_projection)
  ...
  real(8), dimension( 2 * maxvec -2 ) :: scratcha  !COSMIN = 2*maxvec-1
  real(8), dimension(maxvec) :: diag
  real(8), dimension(maxvec) :: offdiag             !COSMIN = maxvec-1  but Lapack needs size maxvec
...
!!$  real(8), dimension(VECSIZE, maxvec), target :: lanc, proj
  ...
  real(8), dimension(VECSIZE) :: newpos, newpos_, newforce, newforce_, ref_force
  real(8) :: ran3
  

  
  !write(*,*) 'TOUT EST BIEN DECLARE.............. size = ',  VECSIZE , maxvec
...
34 continue
  ...
  if(.not. new_projection ) then
     ...
     ! COMMENTED BY KIMIYA 28/03/2008
!!$    xsum = 0.0d0
!!$    ysum = 0.0d0
!!$    zsum = 0.0d0
!!$    do i=1, NATOMS
...
!!$    ysum = ysum / NATOMS
!!$    zsum = zsum / NATOMS
     ! END COMMENTED BY KIMIYA 28/03/2008
...
! END COMMENTED BY KIMIYA 28/03/2008  
  ...
  ! We extract lanczos(1)
   newforce = (newforce - ref_force)     ! COSMIN VERSION


  
  !newforce = (newforce_ - newforce)/(2. * increment)     ! KIMIYA VERSION
