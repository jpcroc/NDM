...
subroutine  diago_scalapack_complex(N,W)
      ...
      use mpi
      use thermo_module
      use diago_scalapack_complex, only : Z, DESCZ
      implicit none
!      include 'mpif.h'

      integer   :: istat,info,i
      complex(kind(1.d0)),dimension(:,:),allocatable  :: a
      complex(kind(1.d0)),dimension(:,:),allocatable  :: c
      ...
      integer   :: root, ierr
      real(kind(1.d0)),dimension(:),allocatable    :: memory_a
      logical :: PRT_MEM
 ...
 !     PDSYEVX interface
      ...
      complex(kind(1.d0)), allocatable, dimension(:) :: work
      complex(kind(1.d0)), allocatable, dimension(:) :: rwork
      real(kind=8),        allocatable, dimension(:) :: gap
...
!
      ...
      if(allocated(ipiv)) deallocate(ipiv) ; allocate (ipiv (n), stat=istat)
      if (istat/=0) stop "ERR:ALLOCATE FAILS for IPIV"


      if(allocated(memory_a)) deallocate(memory_a) ; allocate(memory_a (nprocs), stat=istat)
      if (istat/=0) stop "ERR:ALLOCATE FAILS for memory_a"

!     PDSYEVX interface
      if(allocated(gap)) deallocate(gap) ;   allocate (gap (nprocs), stat=istat)
      if (istat/=0) stop "ERR:ALLOCATE FAILS for gap"
      if(allocated(ifail)) deallocate(ifail) ;  allocate (ifail (n), stat=istat)
      if (istat/=0) stop "ERR:ALLOCATE FAILS for ifail"
      if (allocated(iclustr)) deallocate(iclustr) ;  allocate (iclustr (2*nprow*npcol), stat=istat)
      if (istat/=0) stop "ERR:ALLOCATE FAILS for iclustr"
      if (allocated(ra)) deallocate(ra) ; allocate (ra (nb,nb), stat=istat)
      if (istat/=0) stop "ERR:ALLOCATE FAILS for ra"
      if (allocated(ca)) deallocate(ca); allocate (ca (nb,nb), stat=istat)
...
!
       ...
       call MPI_Gather(memory_local, 1, MPI_COMPLEX8,                      &
     &                memory_a, 1, MPI_COMPLEX8,   root,                   &
     &                MPI_COMM_WORLD, ierr)
...
!
 ...
 orfac   =   0.   ! Ortho
      ...
      if (leigenvectors) then
 	  ...
 	  liwork=6*npq
          if (allocated(iwork)) deallocate(iwork) ; allocate  (iwork (liwork), stat=istat)
          if (istat/=0) stop "ERR:ALLOCATE FAILS for iwork"
          if (allocated(work)) deallocate (work) ;   allocate (work (lwork), stat=istat)
          if (istat/=0) stop "ERR:ALLOCATE FAILS for work"

          if (allocated(rwork)) deallocate (rwork) ;   allocate (rwork (lrwork), stat=istat)
          if (istat/=0) stop "ERR:ALLOCATE FAILS for rwork"
   ...
   if (leigenvectors) then
       if (iam==0) print*,'Eigenvalues and eigenvectors are computed'
        call PZHEEVX('V','A','L',N,A,1,1,desca,vl,vu,imin,imax,  &
                abstol,nvalues,nvectors,W,orfac,Z,1,1,descz,work, &
                lwork,rwork, lrwork,liwork,ifail,iclustr,gap,info )
...
end subroutine  diago_scalapack_complex


subroutine init_my_matrix_complex (context,iproc,nprocs,N,A,NPROW,NPCOL,MYROW,MYCOL,DESCA)
  use mpi
  use mod_mpi_phondy
  ...
  complex(kind(1.d0)) :: AIJ!, MPI_Wtime

  integer :: I,J, jptest,ilocal

  integer :: prank,qrank
  real(8) :: temps_i1,temps_i2,temps_i3,temps_f,temps_b, temps_i4
  complex(kind(1.d0)), dimension(:), allocatable :: mlocali
...
!-----------------------------------------------------------------------
!
  subroutine get_solution_complex (n,i,z,descz,eigvect)
    implicit none
    integer :: n
    integer :: descz(:)
    complex(kind(1.d0)) ::  z(:,:)
    real(kind(1.d0)) :: eigvect(n)
    integer :: i
    integer :: j
