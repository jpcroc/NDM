...
! $Id: art_lanczos.f90 1534 2015-12-14 15:59:38Z mickael $


module lanczos_defs
  use triclinic_pbc_positions_mod
  implicit none
  save
...
subroutine lanczos( maxvec, new_projection, produit )
  ...
  use lanczos_defs
  use random
  use generate_local_region_mod, only: mask_inner_region
  ...
  call calcforce( NATOMS, pos, boxl, ref_force, lanc_energy, evalf_number )
  z0 => lanc(:,1)
  ...
  if ( .not. new_projection ) then
     ...
     if ( iproc == 0 ) then           ! Initial movement.
        do i = 1, natoms, 1
           if ( constr(i) == 0 ) then
              do
                ...
                ! check whether displacement is isotropic
        ...
        ! If we are using local forces, then we muse ensure that the eigenvector is only
        ! non-zero in the inner region
        if(use_local_forces) then
           do i=1, VECSIZE
              z0(i) = z0(i) * mask_inner_region(i)
           end do
        end if
...
!#endif
  ...
  call calcforce( NATOMS, newpos, boxl, newforce, excited_energy, evalf_number ) ! Problem: newforce = ref_force
  ...
  if ( IN_MINIMUN .and. LANCZOS_MIN ) then
     ...
     call calcforce( NATOMS, newpos, boxl, newforce, proj_energy, evalf_number )
  else
