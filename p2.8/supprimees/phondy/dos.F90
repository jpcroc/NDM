...
#else
      eigenvector1(jj_mat)=Z(1+(i_loc-1)*3,jj_mat)
      eigenvector2(jj_mat)=Z(2+(i_loc-1)*3,jj_mat)
      eigenvector3(jj_mat)=Z(3+(i_loc-1)*3,jj_mat)
#endif
    end do
...
! passing argumets ....
    ...
    real(kind=8) :: nu
#else
  subroutine write_ldos_threading (N,W,Z)
    use T_kind_param_m, ONLY:  double
    use phondy_in_ndm_module, ONLY : unit_nu,nu_min,nu_max,n_nu_up,nsitedos,width_dos,isitedos,weight
    use thermo_module
    implicit none
...
#else
 return
 end subroutine write_ldos_threading
#endif
