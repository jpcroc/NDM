...
! lammps_extract_atom {{{2
   ...
   subroutine lammps_extract_atom_dp2a (atom, ptr, name)
      ...
      if ( name /= 'x' .and. name /= 'v' .and. name /= 'f' .and. &
           ...
           name /= 'angmom' ) then
         ! We should not be getting a rank-2 array here!
         call lammps_error_all (ptr, "LAMMPS.F90",406, 'You cannot extract those atom&
...
! lammps_extract_compute {{{2
   ...
   subroutine lammps_extract_compute_dp (compute, ptr, id, style, type)
      ...
      if ( style /= 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",443, 'You cannot pack per-atom/local&
            & data into a scalar.')
         return
      end if
      if ( type == 1 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",448, 'You cannot extract a compute&
            & vector (rank 1) into a scalar.')
         return
      else if ( type == 2 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",452, 'You cannot extract a compute&
   ...
   subroutine lammps_extract_compute_dpa (compute, ptr, id, style, type)
      ...
      if ( type == 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",468, 'You cannot extract a compute&
            & scalar (rank 0) into a rank 1 variable.')
         return
      else if ( type == 2 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",472, 'You cannot extract a compute&
   ...
   subroutine lammps_extract_compute_dp2a (compute, ptr, id, style, type)
      ...
      ! Check for the correct dimensionality
      if ( type == 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",490, 'You cannot extract a compute&
            & scalar (rank 0) into a rank 2 variable.')
         return
      else if ( type == 1 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",494, 'You cannot extract a compute&
...
! lammps_extract_fix {{{2
   ...
   result (fix)
      ...
      if ( (type >= 1 .and. Ci < 0) .or. &
         ...
         call lammps_error_all (ptr, "LAMMPS.F90",523, 'Index out of range in&
            & lammps_extract_fix')
      end if
   ...
   subroutine lammps_extract_fix_dp (fix, ptr, id, style, type, i, j)
      ...
      if ( style /= 0 ) then
         select case (type)
         case (0)
            call lammps_error_all (ptr, "LAMMPS.F90",539, 'There is no per-atom or local&
               & scalar data available from fixes.')
         case (1)
            call lammps_error_all (ptr, "LAMMPS.F90",542, 'You cannot extract a fix''s &
               &per-atom/local vector (rank 1) into a scalar.')
         case (2)
            call lammps_error_all (ptr, "LAMMPS.F90",545, 'You cannot extract a fix''s &
               &per-atom/local array (rank 2) into a scalar.')
         case default
            call lammps_error_all (ptr, "LAMMPS.F90",548, 'Invalid extract_fix style/&
               &type combination.')
         end select
         return
      end if
   ...
   subroutine lammps_extract_fix_dpa (fix, ptr, id, style, type, i, j)
      ...
      ! Check for the correct dimensionality
      if ( style == 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",569, 'You can''t extract the&
            & whole vector from global fix data')
         return
      else if ( type == 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",573, 'You can''t extract a fix&
            & scalar into a rank 1 variable')
         return
      else if ( type == 2 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",577, 'You cannot extract a fix&
            & array into a rank 1 variable.')
         return
      else if ( type /= 1 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",581, 'Invalid type for fix extraction.')
         return
      end if
   ...
   subroutine lammps_extract_fix_dp2a (fix, ptr, id, style, type, i, j)
      ...
      ! Check for the correct dimensionality
      if ( style == 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",599, 'It is not possible to extract the&
            & entire array from global fix data.')
         return
      else if ( type == 0 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",603, 'You cannot extract a fix&
            & scalar (rank 0) into a rank 2 variable.')
         return
      else if ( type == 1 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",607, 'You cannot extract a fix&
...
!-------------------------------------------------------------------------2}}}
   ...
   subroutine lammps_gather_atoms_ia (ptr, name, count, data)
      ...
      if ( count /= 1 .and. count /= 3 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",692, 'lammps_gather_atoms requires&
   ...
   subroutine lammps_gather_atoms_dpa (ptr, name, count, data)
      ...
      Cname = string2Cstring (name)
      if ( count /= 1 .and. count /= 3 ) then
         call lammps_error_all (ptr, "LAMMPS.F90",718, 'lammps_gather_atoms requires&
            & count to be either 1 or 3')
      else
         Ccount = count
      end if
...
!-----------------------------------------------------------------------------
   ...
   subroutine lammps_scatter_atoms_ia (ptr, name, data)
      ...
      if ( Ccount /= 1 .and. Ccount /= 3 ) &
         call lammps_error_all (ptr, "LAMMPS.F90",746, 'lammps_gather_atoms requires&
            & count to be either 1 or 3')
      Fdata = data
      Cdata = C_loc (Fdata(1))
   ...
   subroutine lammps_scatter_atoms_dpa (ptr, name, data)
      ...
      Ccount = size(data) / natoms
      if ( Ccount /= 1 .and. Ccount /= 3 ) &
         call lammps_error_all (ptr, "LAMMPS.F90",765, 'lammps_gather_atoms requires&
            & count to be either 1 or 3')
      Fdata = data
      Cdata = C_loc (Fdata(1))
      call lammps_actual_scatter_atoms (ptr, Cname, Ctype, Ccount, Cdata)
