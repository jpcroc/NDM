...
!             hold(i1,i2)=boxcg%at(i1,i2)
  ...
  subroutine test_conv(Nt,Ft,lover,Vt,Rt,lvm )
    ...
    real(double)::forctot,formax,deltaV,sigmax,fsigmax,FM2,FT2,sigm2,ppot,fsifm2
...
!!$       end do
    ...
    if (present(Vt)) then
       write(unitgc,*)'formax,forctot',formax,forctot
!       select case(ityprel)
!       case(1)
!!$          if(lvm) then
!!$             write(unitgc,'(I4,5E20.11,A, 1E20.11)')ncalls, Vt*erg2eV ,forctot,formax,sigm2,' ****', deltaV
!!$             write(6,'(I4,5E20.11,A, 1E20.11)')ncalls, Vt*erg2eV ,forctot,formax,sigm2,' ****', deltaV*erg2eV
!!$          else
!!$             write(unitgc,'(I4,5E20.11)')ncalls, Vt ,forctot,formax,sigm2
!!$             write(6,'(I4,5E20.11)')ncalls, Vt*erg2eV ,forctot,formax,sigm2
!!$          end if
!!$       case(2)
          if(lvm) then
             write(unitgc,'(I4,5E20.11,A, 1E20.11)')ncalls, Vt ,ft2,fm2,Fsigmax, sigm2, ' ****', deltaV
             write(6,'(I4,5E20.11,A, 1E20.11)')ncalls, Vt*erg2eV ,ft2,fm2,Fsigmax, sigm2, ' ****', deltaV*erg2eV
          else
             write(unitgc,'(I4,5E20.11)')ncalls, Vt ,ft2,fm2,Fsigmax,sigm2
             write(6,'(I4,5E20.11)')ncalls, Vt*erg2eV ,ft2,fm2,Fsigmax,sigm2
          end if
          ftotC=ft2 ; fmaxC=fm2; fisgC=fsigmax
!!$
!!$       end select
       if (lvm)then
          select case(ityprel)
