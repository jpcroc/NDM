...
!          H=F
  ...
  subroutine CGPcrit(N,R,V,F,lover,lorig,beta0)
    use WGC,only:ftotc,fmaxc,fsigc
    integer::N
    real(double)::beta0
    real(double),dimension(:)::R,F
    real(double)::V
    ...
    if (lvarcrit) then
       if (lvarcrit) then
          ...
          case(1)
             ...
             fumstopOR=fsumstop
             fcheck=fpstop*10
             fsumcheck=fsumstop*10
             if(fpstop.gt.0) then
                if (fmaxc.gt.fcheck) then
                   fpstop=fmaxc/10
                end if
             end if
             if(fsumstop.gt.0) then
                if (fsumc.gt.fsumcheck) then
                   fsumstop=fsumc/10
                end if
             end if
             
          case(2)
             fsigcheck=fsigstop*10
             if (fsigc.gt.fsigcheck) then
                fsigstop=fsigc/10
             end if
          end select
       ...
       call conjugategradient(N,R,V,F,lover,lorig,beta0)
    end if
    return
...
!!$         end if
  ...
  subroutine checkline (lover,Ldir,F0,normF02,N,R,V,F,Rt,Vt,Ft)
    ...
    ldir=.false.
    if (lover) then
       write(unitgc,*)'checkline lover R to Rt'
       R=Rt
       V=Vt
       F=Ft
       return
