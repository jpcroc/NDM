...
subroutine readdm
  ...
  lpr=.false.                 ! parinnelo rahman ıÇâÖ„ÅÇ contrainte constante
  ...
  ipotentiel = -1              ! definit type potentiel : 0=Born-Mayer-Huggins, 1=Buckingham, 2=watanabe,3=buck8,4=UO2, 5 terme Morse, 6=SW ıÇâÖœÄÔΩ¥ÔæéÔΩµÁ∏∫ÔΩ§† la Vashista ; 7 pot paire tabule ; 10 EAM; 12 ZrC JuLi(+Tersoff Doan)  ; 13 Tersoff coupure COS; 14 Tersoff coupure FD ; 15 tersoff coupure SIN (original) ; 11 Ercollesi
  npotentiel = 1              ! nb de potentiels
  lpotentiel(:)=.false.
  ntyp=-1                    ! le nombre de type DOIT etre specifie si le nombre de potentiel est superieur ıÇâÖœÄÔΩ¥ÔæéÔΩµÁ∏∫ÔΩ§† 1
  ! PME
  ...
  lFrozen=.FALSE.
  lxfrozen=.FALSE.             ! .true.: certains atomes sont bloqueıÇâÖıÄàö‚ñΩıÇâÑÔΩ∏ (pas de dynamique)
  lyfrozen=.FALSE.             
  lzfrozen=.FALSE.             
  lxyfrozen=.FALSE.             
  lxzfrozen=.FALSE.             
  ...
  Theat=0.0
  Eheat=0.
  ivisu=1    ! format de sortie dans rasmol.f90 : ivisu=1=.mol, ivisu=2=vsim mal codıÇâÖıÄà¶ÔΩΩÔΩ©, ivisu=3=xred
  ...
  decal_bc = 0.
  ldecal_bc = .false.
  itespebcout = -1      ! on n'ıÇâÖıÄà¶ÔΩΩÔΩ©crit pas de .cfg pour le film
  ldyn2D = .false.      ! par dıÇâÖıÄà¶ÔΩΩÔΩ©faut : bords libres selon Y
  !   ----------------------------------------------------------------------------------------    !*!
  ...
  lrctest=.true.
  tcelec=0 ! tempÈ¨ßÔΩªature de coupure pour les pertes È¨ßËÑáctroniques
  Ecelec=0 ! tempÈ¨ßÔΩªature de coupure pour les pertes È¨ßËÑáctroniques
...
!  end if
  ...
  IF (lxFrozen.OR.lyFrozen.OR.lzFrozen) THEN
     ...
     !    i.e. quelles forces doivent ıÇâÖœÄÔΩ¥ıÇâÑÔΩ™tre annulees
     Allocate(Free(1:imm))
     Free(:)=.true.
  ...
  if (ldesinteg)then
     ...
     if (rang==0)write(6,*)
     if (rang==0)write(6,*)'desintegration de l atome ',ides, 'mis ıÇâÖœÄÔΩ¥† 1'
     ...
     if (rang==0)write(6,*)'ATTENTION EN PARA un atome ne doit pas aller d-un proc. ıÇâÖœÄÔΩ¥† un proc non voisin'
