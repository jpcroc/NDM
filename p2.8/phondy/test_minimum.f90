...
subroutine test_minimum !(xp, xpp, vp, ax, fp, ielat, iwmax, iwmax2, ityp)
  ...
  ! MPI
  if (rangph==0) write (6, *) 'PHONDY: Testing if the forces are zero.'
  ...
  vp=0.0
        !do ii=1,im
        !   write(6,*)ii,xp(1,ii),xp(2,ii),xp(3,ii)
        !end do

 fp=0.
  call calfo_phondy(1,1)
  epot0=potist*erg2eV
  if (rangph==0)   write (6, '(" PHONDY: *Epot (eV) = ",D21.12)') epot0
  fp0=fp
!debugCOS  do i=1,im
