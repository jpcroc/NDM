...
contains
  ...
  subroutine read_parameters( )
    ...
    integer :: activation_max_iter,Max_Perp_Moves_Basin,Min_Number_KSteps,Max_Iter_Basin,Lanczos_SCLoop,&
  ...
  character(len=11) :: FILECOUNTER,RESTART_FILE
    ...
    namelist /inputart/setup_initial,guess_noise,GUESSFILE,dim,check_connectivity,local_lanczos,INNER_REGION,&
         ...
         & MAX_LANCZOS_STEPS,temperature,Number_Events, TYPE_EVENTS,Radius_Initial_Deformation,Central_Atom,sym_break_dist,&
         ...
         &delr_threshold,USE_DIIS,ITERATIVE ,&
         ...
         &Type_selected,Dual_system,size_system,event_type,activation_max_iter
    size_system=-1.
    Dual_system=.false.
    Type_selected=0
    coord_number=-1
...
delta_disp_Lanczos=0.01
    ...
    activation_max_iter=100
    sym_break_dist=0
    Central_Atom=-1
    TYPE_EVENTS='undefined'
...
!!$  endif
  ...
  subroutine write_parameters( )
    ...
    if ( iproc == 0 ) then
       ...
       write(flog,*) '****************************** '
       write(flog,*) 'WELCOME TO  ART nouveau (with LAMMPS) '
       write(flog,*) '****************************** '
