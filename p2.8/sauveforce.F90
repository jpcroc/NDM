  ...
  subroutine sauveforce(itapp)
    ...
    if(rang==0) then
       if (itapp==0) then
          ...
          !    pour l'ecriture.
          write (6, *)
          fnamcoutfp = fnam(1:lenfnam)//'.coutforce.'//extension




          write (6, *) 'sauvegarde forces it=', itapp, fnamcoutfp
          open(unit=lucoutfp, file=fnamcoutfp, form='unformatted', status='unknown')
       ...
       write (lucoutfp) ibuffer  ! Ecriture ityp
       !debug     if (rang==0) write(*,*) 'Preparation ecriture ityp'
       write (lucoutfp) buffer   ! Ecriture fp
       !debug     if (rang==0) write(*,*) 'Ecriture fp'
       ...
       enddo
       write (lucoutfp) ibuffer   ! Ecriture num_at_glob

#else
       write (lucoutfp) ityp
       write (lucoutfp) fp
       write (lucoutfp) num_at_glob
#endif
       ...
       call MPI_SEND(fp(1:3,1:im),3*im,NDM_MPI_REAL_DOUBLE,0,12003,MPI_COMM_space,ierr)
