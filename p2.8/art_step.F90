...
! $Id: art_step.f90 1534 2015-12-14 15:59:38Z mickael $
...
module art_step_mod
  ...
  use min_converge_mod,only:perp_fire
!  use save_restart,only:restart
  ...
  While_lanczos: do
      ...
      if ( (ftot < EXITTHRESH) .or. (pas > MAXPAS) .or. (liter > MAX_LANCZOS_STEPS) .or. &
         ...
         if ( ftot < EXITTHRESH ) then
            ...
            ret = 600000 + pas
         else if ( pas > maxpas .or. liter > MAX_LANCZOS_STEPS ) then
            ret = 500000 + pas
         end if
...
subroutine lanczos_step ( current_energy, a1, liter, get_proj )
  ...
  !Local variables
  integer :: i, step_rejected, maxiterfire
  ...
  fpar = dot_product( force, projection )
  pos = pos - sign(one,fpar)*projection* min(2.0d0*INCREMENT,abs(fpar)/max(abs(eigenvalue),0.5d0))
...
!!$  end if
  ...
  !we run FIRE in perpendicular hyperplane

  maxiterfire = min(100,5*liter)
  call perp_fire(minimized,maxiterfire)
  if (.not. minimized) then !if fails, use steepest descent
  ...
  ! we get eigen ddirection for the minimum of this hyperplane.
  if ( get_proj ) then
...
subroutine apply_glisse( giter, saddle_energy )
  ...
  call calcforce( NATOMS, pos, box, force, current_energy, evalf_number )
  fcur = force
  poscur = pos
  ...
  do
     ...
     if (should_reset_velo) then
        ...
        do
           ...
           call calcforce( NATOMS, pos_b, box, force_b, total_energy, evalf_number )
                                      ! New force's components.
           call force_projection_art( fpar_b, perp_force_b, fperp_b, ftot_b, &
                  & force_b, projection )
