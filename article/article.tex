...
binding energy.
Zhang et al.\cite{Zhang201190} studied interaction of He with monovacancy and SIA in pure and Vanadium alloys. 
...
which are reported in the Fig.\ref{fig:vacancies}.  We point here the exception of the di-vacancy cluster. 
...
for which the first nearest neighbor configuration is slightly attractive while the second nearest neighbor 
...
\begin{center}
 \begin{tabular}{ccc}
    ...
    (d) & (e) & (f) \\
 \end{tabular}
\caption{The configurations of vacancy clusters ($V_m$, $m=1, 3, 4,5 $) with the lowest formation energy. 
For the case of di-vacancy we plot the first nearest neighbor ($V_2^{1nn}$) and the second nearest neighbor 
...
The only exception was the di-vacancy for which the two configurations first and second nearest neighbors have been chosen.   
%number of choices ... 
...
He atoms on $O$ sites \wes{in both V and Ta case}.
\MCM{The same question as previous}.
In V, for di-vacancy clusters, He$_1V_2$,  the $1nn$ configuration of di-vacancy is still higher in energy 
...
systematically less than for the 2nn case. Futhermore, the difference is larger when adding more He atoms.}
...
whereas He-vacancy binding energy is 1 order of magnitude larger (Fig.\ref{fig:eb_he}). Hence, adding a He atom result in adding one more 
...
For practical use, e.g. in cluster dynamics simulations,  in first order approximation are important only the upper value of those binding energy. The values are extracted in the table Tab.~\ref{tab:ebhe}.  
...
\wes{But for Tantalum, the binding energy of vacancy is likely to be linear at lest up to $m$=4 with E$^b$ = 0.97 eV.}
