...
for both Vanadium ($E_{V_1}^f$=2.47 eV) \cite{maier1979,janot1982} 
and Tantalum ($E_{V_1}^f$=2.88 eV) \cite{maier1979,schultz1982,Schaefer1987}.}
...
For mixed configurations He$_1V_1$ in Vanadium, the  lowest energy configuration is given  by the He atom 
...
\begin{center}
 \begin{tabular}{ccccc}
    ...
    & (d) & (e) & (f) & \\
 \end{tabular}
 \caption{Lowest formation energy He$_nV_1$ type clusters in Vanadium and Tantalum. 
...
\wes{One can notice in case of He$_3V_1$ cluster, He atoms form an equilateral triangle centered on the vacancy in V, 
...
%
\wes{Adding more He in interstitial clusters He$_n$ at least up to $n$=4, amplifies lattice distorsion so that 
...
Corrolary, in Vanadium, H$_{20}V_4$ clusters which are clearly unstable with negative binding energy, 
...
% \centering
 ...
 \end{tabular}
 \caption{Binding energy of mono-vacancy to He$_nV_m$ clusters as a function of \textit{m} in Vanadium (left panel) 
...
We computed 2 paths for interstitial diffusion barrier : $T$ site to $T$ site, and $T$ site to $O$ site.\MCM{Why you have chosen those paths? These migration paths govern the migration oh He in bcc matrix?}
%
$T$ $\rightarrow$ $T$ path shows a barrier of 0.12 eV (Tab.\ref{tab:em}) in good agreement with TDS experiment in vanadium which givee the value of  0.13 eV~\cite{Fedorov1996}.
...
The DFT migration barrier of the mono-vacancy has the value of 0.48 eV and  agrees well with TDS experiments of Fedorov \cite{Fedorov1996}, 0.5 eV,  and Vanveen,  0.47 eV, \cite{vanveen1994}.\MCM{There is particular temperature for these experiments?}. The di-vacancy migration barrier from the $V_2^{1nn}$ configuration to $V_2^{2nn}$ configuration is 
0.34 eV. Di-vacancy cluster $V_2^{2nn}$ configuration is more mobile \MCM{More mobile that what ?} since it can migrate without dissociation with a barrier of 0.37 eV.

We turn on the influence of He on vacancy diffusion. 
%
For this goal, NEB was used in order to search for the minimum energy path of He$V_1$ cluster having in the initial and final state of the diffusion process the corresponding lowest energy configurations. 
\MCM{ Wesley this part isunclear. I cannot rewrite because I do not understand: Ther migration path showed first the dissociation of He to the monovacancy. Then He atom diffuse to a position near tetrahedral interstitial, 
...
%
The \MCM{experimental, theoretical, DFT ?} migration barrier is about 1.27 eV that correspond roughly to the He-vacancy binding energy plus migration barrier of He ($T$ $\rightarrow$ $T$). As a result, He increases the diffusion barrier of mono-vacancy by 0.67 eV.

The di-vacancy $V_2^{1nn}$ can migrate into $V_2^{2nn}$ configuration with 0.79 eV barrier. 
Migration from $V_2^{2nn}$ to $V_2^{2nn}$ configuration is a 2 steps process with a lower barrier 0.66 eV \MCM{compared with what?}. This process is a two step mechanism. Firstly, one of the vacancy of the cluster dissociates from the cluster to reach first nearest neighbour site \MCM{ in the $\langle 111 \rangle$ direction}  changing the geometry of the cluster into a slightly shifted $V_2^{1nn}$ configuration. Secondly, the second vacancy moves to its nearest neighbour \MCM{in the same  $\langle 111 \rangle$ direction}. In trhe same time the He atom goes to its final $0$ position 
between the 2 vacancies \MCM{where it is the He in the initial position. There is hardly to figure out, without direction, the initial configuration to guess this migration mechanism. Even you repeat about configuration for clarity in a necessity, of put a reference on the figures that you already have.} The trapped He atom almost doubles the migration barrier of the di-vacancy\MCM{for 1nn, 2nn, both?}.
Nevertheless, the presence of He does not change the relative mobility of mono and di-vacancy but moreover, it amplifies the gap \MCM{the gap between what and what}.
...
The migration barrier of interstitial He in Ta lattice are reported in the (Tab.\ref{tab:em}). The migration barrier of interstitial He are 
0.18 eV and 0.40 eV for the mechanisms $T$ $\rightarrow$ $T$  and $T$ $\rightarrow$ $O$, respectively.
The lowest energy migration mechanism correspond to the $T$ $\rightarrow$ $T$ ad is similar to the V case.  

The value of migration barrier of mono-vacancy in Ta is 0.73 eV.
This result is consistent with resistivity annealing experiment which give the value of 0.7 eV~\cite{faber1974}. The lowest migration barrier of di-vacancy correspond to the 2nn $\rightarrow$ 2nn path. Similar to V the last mechanism is more favorable (with 0.17 eV) than the migration of the mono-vacancy.\MCM{These results confirm the tendency of for all bcc metals, Fe, W etc bla bla}

Diffusion of mixed cluster He$_1V_1$ involve firstly  the dissociation of He from the vacancy\MCM{where? in which direction 1nn, 5nn etc}. Subsequently,  the He atom hops via $T$ interstitial sites while the 
vacancy jumps to first nearest neighbour site\MCM{The $V_1$ and He migrate in the same time ? }. Finally, He recombines with the mono-vacancy, 
following the 'impeded interstitial mechanism' or 'He-vacancy dissociative mechanism' \cite{Trinkaus2003,ullmaier1991} \MCM{Unclear, can you detail this mechanism?} 
The presence of the He atom enhances the migration barrier of mono-vacancy by 1.17 eV.  For the He$_1V_2$ case, the increase of migration energy due to He is 2 times smaller (0.51 eV) for He$_1V_2^{1nn}$ $\rightarrow$  He$_1V_2^{2nn}$ path  than for He$_1V_1$ case.

Contrary to the V case all paths involving He$_1V_2$ clusters have the almost same barrier in presence of He. Consequently, He$_1V_2$ clusters can migrate via three ways with the same probability \MCM{Are you sure that are only three ways ? Maybe there are three channels  but did you know the degeneracy of those channels ? Fix that.  }
...
of He atom or vacancy\MCM{citation of general textbook}:
\begin{equation}
    E^d_{He/V-He_nV_m} = E^b_{He/V-He_nV_m} + E^m_{He/Va}
\end{equation}
\MCM{is the first time when you use symbols like $He/V-He_nV_m$ can you explain which is which ?}
In Fig.\ref{fig:ed} is shown for V and Ta the dissociation energy of He and vacancy with respect to the lowest energy configuration of He$_nV_m$ cluster as a function of \textit{n/m} ratio. 
%
...
%
In  Fig.\ref{fig:ed} the crossover \MCM{between what and what} shows the most stable clusters for $\sim$ 0.9 \textit{n/m} ratio with E$^d$ $\sim$ 1.9 eV in V.  This theoretical result is close to experimental result which indicate the E$^d$ = 1.66 eV~\cite{vassen1991},
The corresponding quantities for Ta are estimated to \MCM{where is n/m}  and   E$^d$ $\sim$ 2.5 eV.
%
...
clusters with \textit{n}=\textit{m}\MCM{for V?  Ta? both?}. 
...
Comparing the energy landscape of He in the neighborhood of the vacancy in V and Ta, it can be notted that the $S$ site is attractive in Ta 
while it is repulsive in Vanadium (Fig.\ref{fig:henv1}--\ref{fig:henv2}). Similar result has been found \cite{seletskaia2008} which explain this tendency by a larger charge density at vacancy in V which energy is close to the Fermi level \MCM{which energy?}, thus closed-shell He will have a repulsive interaction \MCM{unclear. Details}. Indeed, He at $S$ site in V shows a larger density of state at the Fermi energy level than in Ta 
...
He is shifted toward $O$ site.\MCM{This section should be suppressed. Probably  you must put all this paragraph when you discusses He$_1V_m$ clusters. You can leave this paraggraph only is you discuss all the results, type of clusters etc}.
...
\begin{itemize}
 ...
 \item Formation of Frenkel pair is expected with growing interstitial He$_n$ clusters \MCM{growing of interstitial clusterS or interstitial cluster i.e. increasing $n$?. Fix that}, since the formation energy of He$_nV_1$ cluster is less than pure interstitial He$_n$ cluster for $n\ge3$ in V and $n\ge2$ in Ta.
\MCM{be more clear here. You put effect then cause. The natural order is cause and then effect}. 
 \item In V, He atoms prefer to occupy interstitial sites contrary to Ta case for which He atoms reside in substitutional sites because of larger density of state \MCM{DOS of what?} at Fermi level for V than for Ta.
 
 \item  for both V and Ta, the formation energy of di-vacancy in $V_2^{2nn}$ configuration  is  lower than the formation energy of $V_2^{1nn}$ configuration. This tendency id observed also in the $VI$ metals such as Mo and W. The di-vacancies in V and Ta have a weak attractive binding energy whereas are repulsive for group $VI$ metals.
 
 \item The effect of He on di-vacancies is to increase the formation energy of the  $V_2^{2nn}$configuration much more than the  $V_2^{1nn}$ one 
 so that the hierarchy is reversed for $n\ge2$ in V and $n\ge1$ in Ta\MCM{Many idea in the same phrase. Please rewrite simple and efficient}. In addition, He atom makes the 1nn configuration more 
 stable than the 2nn configuration as the binding energy of He$_nV_2^{1nn}$ to He atom and mono-vacancy is larger than 
 for He$_nV_2^{2nn}$ clusters \MCM{The same point as previous.Keep short simple, exact. Rewrite the above part}. 
 
 \item as general tendency  the He atom stabilizes vacancy type clusters and vice versa.
 
 \item For the 2 studied metals of group $VB$, di-vacancy is more mobile than mono-vacancy. The presence of He atom increase the value of migration energy barrier of both mono-vacancy and di-vacancy but also their relative difference\MCM{I do not understand. If the values are bigger also the differences are. Rewrite}.
 
 \item In absence of He, the diffusion of $V_2$ cluster exhibits an asymmetry in migration energy since the 1nn $\rightarrow$ 2nn path has a lower barrier than the 2nn $\rightarrow$ 1nn path for V and Ta\MCM{As long as the energy is different I do not see how the migration path should be symmetric !?}. 
 Anyway, di-vacancy diffuses preferentially via 2nn $\rightarrow$ 2nn path in a 2 steps vacancy dissociation-recombination process.
 ...
 For the di-vacancy diffusion in V, the 2nn $\rightarrow$ 2nn path is still favored, but in case of Ta, all paths have the same energy barrier.
