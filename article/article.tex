...
the $S$ site is attractive in Ta while it is repulsive in Vanadium (Fig.\ref{fig:henv1}--\ref{fig:henv2}). 
Similar result has been found \cite{seletskaia2008} which explain this tendency by a larger charge density at vacancy 
in V which energy is close to the Fermi level \MCM{which energy?}, thus closed-shell He will have a repulsive interaction 
\MCM{unclear. Details}. Indeed, He at $S$ site in V shows a larger density of state at the Fermi energy level than in Ta 
then increasing the formation energy enough to make this configuration unstable. 
As a result in the most stable configuration, 
...
We computed 2 paths for interstitial diffusion barrier \wes{that connect the lowest energy configurations}: 
$T$ site to $T$ site, and $T$ site to $O$ site.
\MCM{Why you have chosen those paths? These migration paths govern the migration oh He in bcc matrix?}
%
...
\wes{The reverse way is less favored since the energy barrier is 0.58 eV.
Another path passing directly from a $V_2^{2nn}$ configuration to another $V_2^{2nn}$ configuration shows a barrier of 0.37 eV.}
% Di-vacancy cluster $V_2^{2nn}$ configuration is more mobile \MCM{More mobile that what ?} since it can migrate with a barrier of 0.37 eV.
...
\MCM{ Wesley this part isunclear. I cannot rewrite because I do not understand}
\wes{At first step, the He atom diffuses from initial $O$ site to final 3rd nearest neighbour $O$ site, hopping through
1st nearest $T$ sites. In a second time, the vacancy migrates along $\langle$111$\rangle$ direction to reach 
its final position corresponding to the initial configuration (Fig.\ref{fig:henv1} a).}
% The migration path showed first the dissociation of He to the monovacancy. 
% Then He atom diffuse to a position near tetrahedral interstitial, 
...
%
The \MCM{experimental, theoretical, DFT ?}\wes{computed} migration barrier is about 1.27 eV that correspond roughly to the He-vacancy 
...
\wes{Compared to the $V_2^{1nn}$ $\rightarrow $ $V_2^{2nn}$ path, migration from $V_2^{2nn}$ to $V_2^{2nn}$ configuration 
has a lower barrier (E$^m$=0.66 eV)}. The latter process is a two step mechanism. 
Firstly, one of the vacancy of the cluster dissociates from the cluster to reach first nearest neighbour site 
\MCM{ in the $\langle 111 \rangle$ direction}  changing the geometry of the cluster into a slightly shifted $V_2^{1nn}$ 
configuration (Fig.\ref{fig:henv2} a). Secondly, the second vacancy moves to its nearest neighbour 
\MCM{in the same  $\langle 111 \rangle$ direction}. In the same time the He atom goes to its final $O$ position 
between the 2 vacancies where it is in the initial configuration (Fig.\ref{fig:henv2} e).  
\MCM{There is hardly to figure out, without direction, the initial configuration to guess this migration mechanism. 
Even you repeat about configuration for clarity in a necessity, of put a reference on the figures that you already have.} 
\wes{The trapped He atom roughly doubles the migration barrier of the di-vacancy for both
...
it amplifies the gap \wes{between mono-vacancy and di-vacancy energy barrier}.
...
The migration barrier of interstitial He in Ta lattice are reported in the (Tab.\ref{tab:em}). 
The migration barrier of interstitial He are 0.18 eV and 0.40 eV for the mechanisms $T$ $\rightarrow$ $T$ 
and $T$ $\rightarrow$ $O$, respectively.
...
Similar to V the last mechanism is more favorable (with 0.17 eV) than the migration of the mono-vacancy.
% \wes{These results confirm the tendency of for all bcc metals, Fe \cite{fu2005nat}, W, for which migration energy is lower for di-vacancy 
% than for mono-vacancy.}
\MCM{These results confirm the tendency of for all bcc metals, Fe, W etc bla bla}

Diffusion of mixed cluster He$_1V_1$ involve firstly  the dissociation of He from the vacancy
\wes{to reach the 1st nearest $T$ site}. Subsequently,  the He atom hops via $T$ interstitial sites while 
\wes{at the same time}, the vacancy jumps to first nearest neighbour site\MCM{The $V_1$ and He migrate in the same time ? }. 
\wes{Finally, He recombines with the mono-vacancy. This corresponds to the 'impeded interstitial mechanism' or 
...
\MCM{Unclear, can you detail this mechanism?} 
...
the sum of binding energy and migration energy of He atom or vacancy \cite{Morishita2003}\MCM{citation of general textbook}:
\begin{equation}
...
\wes{In the previous equation, $E_{He/V}$ denotes the corresponding energy related to He or vacancy.}
...
%
In  Fig.\ref{fig:ed} the crossover \wes{between dissociation energy of He or vacancy} shows the most stable clusters 
for $\sim$ 0.9 \textit{n/m} ratio with E$^d$ $\sim$ 1.9 eV in V.  
This theoretical result is close to experimental result which indicate the E$^d$ = 1.66 eV~\cite{vassen1991},
The corresponding quantities for Ta are estimated to \wes{$\sim$ 0.9 \textit{n/m} ratio}  and   E$^d$ $\sim$ 2.5 eV.
%
This is consistent with TDS experiment \cite{Fedorov1996} in which the peaks at the highest temperature were attributed 
to the clusters with \textit{n}=\textit{m}\wes{for V}. 
...
\begin{itemize}
 ...
 \item \wes{the formation energy of He$_nV_1$ cluster is less than pure interstitial He$_n$ cluster for $n\ge3$ in V 
...
%  in substitutional sites because of larger density of state \MCM{DOS of what?} at Fermi level for V than for Ta.
 ...
 \item \wes{the effect of He on di-vacancies is to increase the formation energy of the $V_2^{1nn}$ and $V_2^{2nn}$ configurations.
 ...
 \MCM{The same point as previous.Keep short simple, exact. Rewrite the above part}
 ...
 The presence of He atom increase the migration energy barrier of both mono-vacancy and 
 ...
 \item \wes{in absence of He, di-vacancy diffuses preferentially via $V_{2}^{2nn}$ $\rightarrow$ $V_{2}^{2nn}$ path 
 in a 2 steps vacancy dissociation-recombination process.}
