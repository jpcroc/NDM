...
It is useful to set specify in the din file :  
...
Consider setting :
lperiod=.false. That allows the atomic positions to "get out" of the box. This is only apparent as periodic Boundary Conditions are always enforced, but this can be useful if one wants to compare two configurations (e.g. before and after relaxation; initial or final NEB images). NEB preparation MUST be performed with lperiod=.false.  
...
See DINfile.md for additional options pertaining to each algo.  
...
lpcon2 = .FALSE. if true a damping term is added on the dynamics of the box. Provs useful to stabilize the box.  
lpconxyz = .FALSE. T: the relaxation is allowed only along the X, Y and Z axis  
