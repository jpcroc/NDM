...
<---  16  SUNDAE  
  ...
  19 -> thermodynciamcs from force matrix diagonalization  
  15 -> Path Monte-Carlo calculation (==151)  
  151 -> Path Monte-Carlo calculation  
  OTHER MCC are not documented  
...
noz = -1 at the opposite they can be specified in din file. Their maximum valie being the calcuated value. This can be useful to avoid nasty value that would spoil paralelization (e.g. prime number)  
...
lpr=.false.  ! forces parinello-Rahman calculation for dynamcsi or quenching
...
iterasmol = -1  \>0 formatted poutput every iterasmol steps  
iteangle = -1  formatted angular position  
 if \>=0 also plotted at first and last step 
...
debyetemp=-1
lprtpot=.false.
ibrake =0if =1 electronic slowing for cascades (acting on all atoms)
ngrdel=500
