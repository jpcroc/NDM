...
subroutine fft_inter (char1)
...
#ifdef para2c
#ifdef ixia
  RECORD / DXML_Z_FFT_STRUCTURE_3D / FFT_STRUCT
#endif
#endif
  ...
  if (char1.eq.'cald') then
     STATUS = ZFFT_INIT_3D (kpmex,kpmey,kpmez,FFT_STRUCT,.TRUE.)
     CALL ZFFT_APPLY_3D ('C','C','B', qgrid, qgrid, kpmex,&
          kpmey, FFT_STRUCT,1,1,1)
     STATUS = ZFFT_EXIT_3D (FFT_STRUCT)
  endif

  if (char1.eq.'cali') then
     STATUS = ZFFT_INIT_3D (kpmex,kpmey,kpmez,FFT_STRUCT,.TRUE.)
     CALL ZFFT_APPLY_3D ('C','C','F', qgrid1, qgrid, kpmex,&
          kpmey, FFT_STRUCT,1,1,1)
     STATUS = ZFFT_EXIT_3D (FFT_STRUCT)
  endif

#else
  write(6,*) 'Pas de version FFT parallele disponible'
  stop
#endif
#else

  if (char1.eq.'init') then
  ...
  if (char1.eq.'cali') then
     call fftback (nfft1,nfft2,nfft3,kpmex,kpmey,kpmez,kpme,&
          ntable,table,qgrid)
  endif
