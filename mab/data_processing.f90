...
open(unit=996,file='Free_energy_mollifiée_ABFee',status='unknown')!A_tilde
...
do i_loop=-nhisto1,nhisto+nhisto1
  ...
  exp_A_bar(i_loop)=exp_A_bar(i_loop)+exp(-(x_mol(i_loop)-x_mol(i_iter))**2/(2.d0*eta_ABFee*temperature)+A_ee(i_iter)/temperature)*delta_z
  enddo
enddo
...
do i_loop=-nhisto1,nhisto+nhisto1
   write(999,*),x_mol(i_loop)/A2cm,Free_energy(i_loop)*erg2eV
enddo
