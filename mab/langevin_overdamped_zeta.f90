subroutine langevin_overdamped_zeta()
   ...
   USE mab_in_ndm_module, only: dtlang,abf_mode,block,gamma,dcsi, &
                                ...
                                xi_max, xi_min,dtlang_ini, crit_langevin_zeta
...
!     dcsi=1.0
  ...
  if (abf_mode==2) then
    crit_langevin_zeta = 0.05
    dtlang_zeta=1.0*crit_langevin_zeta**2*gamma*m_i(1,1)/(6.d0*temperature)
...
!            (potist-ene0)*dtlang_zeta/(gamma*m_i(1,1))/sqrt(2.d0*temperature*dtlang_zeta/(gamma*m_i(1,1)))
  ...
  if (abf_mode==22) then
     crit_langevin_zeta=0.01
      dtlang_zeta=crit_langevin_zeta**2*(gamma*m_i(1,1))/(6.d0*(temperature/dcsi))
  end if 
