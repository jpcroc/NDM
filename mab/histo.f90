...
!The border zone. Over this border the mean force is zero
 if ((icsi >= -nhisto2).and.(icsi <= nhisto+nhisto2)) then
   histo_xi2(icsi)=histo_xi2(icsi) + 1
  ! The intermediate region 
   if ((icsi >= -nhisto1).and.(icsi <= nhisto+nhisto1)) then
    histo_xi1(icsi)=histo_xi1(icsi) + 1
    ! The inner regin  
     if ((icsi >= 0).and.(icsi <= nhisto)) then
...
subroutine fill_final_histo()!To create the final histogram for the simulation
 ...
 USE mab_in_ndm_module, ONLY: nhisto,nhisto1,nhisto2,& 
                              ...
                              histo_xi,histo_xi1,           &
                              histo_equi,abf_type,abf_mode, histo_zeta,&
                              x_mol,KtoErg,unit_histo2,temperature

implicit none
integer::i_iter
real(double)::sum_histo1,sum_histo,sum_histo_zeta
...
if (histo_equi .eqv. .True.) then
   open(unit=989,file='histogram_xi1',status='unknown')
   open(unit=990,file='histogram_xi',status='unknown')

   sum_histo=sum(histo_xi)
