...
!------
        ...
        write(*,*) "Testing gzip-compressed writing"
        call system_clock(start_time, count_rate)
        call file_write%init("test_gzip.txt.gz", "w")
        do i=1, lines_count
        ...
        write(*,*) "Testing gzip-compressed reading"
        call system_clock(start_time, count_rate)
        call file_read%init("test_gzip.txt.gz", "r")
        do i=1, lines_count
        ...
        call file_write%init("rewind.txt.gz", "w")
        ...
        call file_write%close
        call file_read%init("rewind.txt.gz", "r")
        call file_read%read_line(line)
    ...
    subroutine test_append
        ...
        call file%init("append.txt.gz", "w", stat=stat)
        ...
        call file%init("append.txt.gz", "a", stat=stat)
        ...
        call file%init("append.txt.gz", "r", stat=stat)
...
!------
    ...
    subroutine test_bad_files
        ...
        use dk_gzipfilereader, only: GzipFileReader
!------
        type(GzipFileReader), allocatable :: reader
        type(FException) :: stat
        character(:), allocatable :: line
        integer :: n
!------
        ...
        allocate(reader)
        call reader%open_file("corrupted.gz", "r", stat)
        call reader%read(line, n, stat)
        deallocate(reader)
        ...
        allocate(reader)
        call reader%open_file("incomplete.gz", "r", stat)
        call reader%read(line, n, stat)
        deallocate(reader)
        call check(stat=="Unexpected end of file.", "incomplete file", stat%string())
        call stat%discard
#endif
    end subroutine
end program
