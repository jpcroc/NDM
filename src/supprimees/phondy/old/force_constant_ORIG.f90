...
subroutine force_constant ()
 ...
 ! integer  :: ielat(imm)
  ...
  !$ integer :: nb_taches,orank
  !$OMP THREADPRIVATE (potist,potisrep, potisglue,potiseam)
  !$OMP THREADPRIVATE (xp, xpp, vp, ax, fp, ielat, iwmax, iwmax2, indi2, ityp,at,bg)  
 !-----------------------------------------------
  !
  !
  ! calcul des phonons
...
!debugCOSwrite(*,*)erg2eV/angst
 ...
 !
  ...
  write(*,*) 'xpref out', xpref(1,1)
  write(*,*) 'xpref out', xpref(1,501)

  !$OMP PARALLEL                      & 
  !$OMP DEFAULT (none)                 &
  !$OMP PRIVATE(i,j,nb_taches,orank,u,v)     &
  !$OMP FIRSTPRIVATE(dxp,rtest2,fpm1,fpp1)  &
  !$OMP COPYIN (potist,potisrep, potisglue,potiseam)  &
  !!$OMP COPYIN (xp, xpp, vp, ax, fp, ielat, iwmax, iwmax2, indi2, ityp,at,bg) &
  !$OMP FIRSTPRIVATE (iw,iw1,iw2,iti,jb,ia,deltax,xpnp) &
  !$OMP SHARED (im,epot0, cm)  &
  !$OMP SHARED (xpref, matfor) &
  !$OMP SHARED(rue2)  
   nb_taches= OMP_GET_NUM_THREADS ()
   orank=OMP_GET_THREAD_NUM()
  !$OMP DO SCHEDULE(STATIC,im/nb_taches) 
  do j=1,im

  iti=ityp(j)
  iw1=iw2+1
  iw2=iwmax2(j)
   if ((mod (j,10)==0)) write(*,*) 'rank.......j ',nb_taches, orank,j
   if ((mod (j,10)==0)) write(*,*) erg2eV,ang2cm
   write(*,*) 'rank.......j ',orank,  j
     ...
     do jb=1,3
        ...
        xp(jb,j)=xpref(jb,j)+deltax
        if (orank==0) write(*,*) '11  ', j,xp(1,1),xpref(1,1)         
        if (orank==1) write(*,*) '12  ', j,xp(1,1),xpref(1,501)         
        call calfo
        if (orank==0) write(*,*) '21  ', j,xp(1,1),xpref(1,j)         
        if (orank==1) write(*,*) '22  ', j,xp(1,1),xpref(1,j)    
        if (orank==1.or.orank==2) write(*,*) 'rank1 ...', orank, potist*erg2eV,epot0
        ...
        call calfo
         ...
         if (orank==1.or.orank==2) write(*,*) 'rank2 ...', orank, potist*erg2eV,epot0
