...
real(8), dimension(:), allocatable :: diag_lbfgs,w_lbfgs
...
subroutine  driver_lbfgs(dim_n)
 use  lbfgs_teta
      implicit none
      real(8), dimension(:), allocatable :: x_coord,grad_functx
      real(8)  ::  functx
      integer ::  dim_n
!
...
subroutine set_up_the_lbfgs(N)
 ...
 if (allocated(w_lbfgs)) deallocate (w_lbfgs)
 allocate(w_lbfgs(nwork))
...
!     ----------------------------------------------------------------------
      ...
      !use lbfgs_teta, ONLY: GTOL,STPMIN,STPMAX,MP,LP
      use lbfgs_teta, ONLY: GTOL,LP
...
!
      ...
      !use lbfgs_teta, ONLY : GTOL,STPMIN,STPMAX,MP,LP
      use lbfgs_teta, ONLY : GTOL,STPMIN,STPMAX,LP
