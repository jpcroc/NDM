...
subroutine cspline(n, x, y, b, c, d)
  ...
  !-----------------------------------------------
  integer , intent(in) :: n
  real(double) , intent(in) :: x(0:n)
  real(double) , intent(in) :: y(0:n)
  real(double) , intent(inout) :: b(0:n)
  real(double) , intent(inout) :: c(0:n)
  real(double) , intent(inout) :: d(0:n)
  !-----------------------------------------------
...
! write(6,*)'spline'
  ...
  if (n>=2) then
     ...
     d(0) = x(1)-x(0)
     c(1) = (y(1)-y(0))/d(0)
     d(1:nm1) = x(2:nm1+1)-x(1:nm1)
     b(1:nm1) = 2.*(d(:nm1-1)+d(1:nm1))
     c(2:nm1+1) = (y(2:nm1+1)-y(1:nm1))/d(1:nm1)
     c(1:nm1) = c(2:nm1+1)-c(1:nm1)
     !
...
!        write(6,*)i,c(i),n,c(i-1),t
 ...
 !       write(6,*)i,c(i),n,c(i-1),t
     ...
     b(:nm1) = (y(1:nm1+1)-y(:nm1))/d(:nm1)-d(:nm1)*(c(1:nm1+1)+2.*c(:nm1))
     d(:nm1) = (c(1:nm1+1)-c(:nm1))/d(:nm1)
     c(:nm1) = 3.*c(:nm1)
     c(n) = 3.*c(n)
     d(n) = d(n-1)
     return
...
subroutine cspline1(n, x, y, b, c, d)
  !-----------------------------------------------
  !   M o d u l e s
  !-----------------------------------------------
  USE T_kind_param_m
