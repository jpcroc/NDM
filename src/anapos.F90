module posana
  ...
  USE gen_com_m, only:uwrt,lwrt, fnam,rang,lperiod,pi,dmtype,lenfnam,iteration,timel,ivisu,unit6P
...
contains
  ...
  subroutine initanapos(atana0,celana0,boxana0)
    ...
    ndvmin=0
    ldepla=.false.
    if (rang==0)write(uwrt,*)'*** analyse du crystal'
    call newunit(unitana)
    open(unitana, file='analyse.in')
    read(unitana,nml=analyse)

    if (ldepla) then
       if (rang==0) write(uwrt,*)'detection des deplacements par rapport a atana0'
       !       if (.not.present(atana0)) then
       !          if (rang==0) write(uwrt,*)'atana0 pas definie STOP'
       !          call arret_ndm
       !       end if
       atrefdep=atana0
       celrefdep=celana0
    ...
    if (lcomp) then
       if (immcr==-1) then
          if (rang==0) write(uwrt,*)'IMMcr must ne specified'
          call arret_ndm
       end if
    ...
    if ((idistord.gt.0).and.(.not.lperiod)) then
       write(uwrt,*) 'distord seulement avec lperiod =true.'
       call arret_ndm
    end if
    if ((idistord.gt.0).and.(.not.lperiod)) then
       write(uwrt,*) 'distord ne fonctionne pas '
       call arret_ndm
    end if
    if ((idistord.ge.3).and.(maxval(pstmax)==0))then
       write(uwrt,*) 'isdistrod=3 preciser pstmax'
       call arret_ndm
    end if
    if(namecr=='ZZ') then
       namecr=fnam(1:lenfnam)
    end if
    ! write(uwrt,*)'distordflag',distordflag
...
!!$    end if
    ...
    if (lcomp) then
       ...
       case(0)
          ...
          if (rang==0)write(uwrt,*)'fnamcr ',len(fnamcr),fnamcr
          ...
          if (rang==0)write(uwrt,*)'fnamperfdef ',len(fnamperfdef),fnamperfdef
       ...
       case default
          write(uwrt,*)'set igencr to 1 or 0 for .crcin or .crgin file respectively'
          call arret_ndm
       end select
       call caltabtc(celcr,atcr,lperiod,boxcr,lchktrav=.false.)
       !       write(uwrt,*)' celana0 celcr celperfdef ',celana0%nox(1),celcr%nox(1),celperfdef%nox(1)
       if (lws) then
          if (rang==0)write(uwrt,*)'analyse de Wigner-Seitz'
       else
          if(ldeptest) then
             if (rang==0)write(uwrt,'(A,F6.1)') 'seuil deplacement pour detection de defauts ',tdep
             tvac=tdep
          end if
          if(lpdep) then
             if (rang==0)write(uwrt,'(A,F6.1)') 'ecriture des deplacés ',tdep
          end if
          tint=tvac
          if (rang==0)write(uwrt,'(A,F6.1)') 'seuil lacune ',tvac 
          if (rang==0)write(uwrt,'(A,F6.1)') 'seuil interstitiel ',tint 
          tdep=tdep*1.0d-8
          tvac=(tvac*1.0d-8)
          tint=(tint*1.0d-8)
       end if
...
!!$    end if
       ...
       if (ldecal) then
          if (idecal.gt.0) then
             if (idecal.gt.atana0%im) then
                write(uwrt,*)'idecal >atcf%im ; stop'
                call arret_ndm
             end if
             do ic=1,3
       ...
       if (ldepla) then
          if (rang==0)write(uwrt,'(A,F6.1)') 'test depla ',tdep
          tdep=tdep*1d-8
       end if
    end if
    close (unitana)
...
!!$         &lsubc,ivisuana
    ...
    if (lcomp)then
       if(ldetdec) then
          ...
          write(uwrt,*)deltx*1d8,delty*1d8,deltz*1d8
          call arret_ndm
       end if
...
!!$       end if
  ...
  subroutine depladet(atc,celc,boxc,atr,celr,boxr,tdep,ndep,inddep,vectdepla,valdepla)
    ...
    if(atc%im.ne.atr%im) then
       write(uwrt,*)'not depladet works for confs with equal number of atoms',atc%im,atr%im
       call arret_ndm
    end if
  ...
  subroutine nbvois(atcf,boxcf,celcf,icall,nbvoisparf,itapp)
    ...
    case(3)
       ...
       if (natd .NE. 0) then
          ...
          atcf%lgul=.false.
          write(uwrt,*)natd,' atomes distordus'
          do iti=1,ntyp
             if (natdtyp(iti).ne.0)write(uwrt,*)natdtyp(iti),' atomes distordus de type ',iti
          end do
          if (itapp.gt.0) then
             lenfn = 9
          ...
          write(uwrt,*) 'Pas d atomes distordus'
       end if
    ...
    do iatvi=1,20
       if(natvi(iatvi).ne.0) then
          write(uwrt,*)
          write(uwrt,*)'Nb d_at. avec',iatvi,'vois. =',natvi(iatvi)!
          do iti=1,20
             if (natvityp(iatvi,iti).ne.0) write(uwrt,*)'Nb d_at. de type',iti,' avec',iatvi,'vois. =',natvityp((iatvi),iti)
          end do
       end if
    end do
    select case (iprtnvi)
...
!!$  end subroutine depcr
  ...
  subroutine ws(atc,celc,boxc,atr,celr,boxr,lws)
    ...
    immax=max(atc%im,atr%im)
    if (lws) then
       if (rang==0)write(uwrt,*)'comparison with reference structure WIGNER SEITZ', atc%im,atr%im,immin,immax
    else
       write(uwrt,*)'comparison with reference structure cut-off radius', atc%im,atr%im,immin,immax,tvac
       if (ldeptest) then
          if (rang==0)write(uwrt,*)'comparison based on displacements', tdep
       end if
    end if
    ...
    if (lvac) then 
       ...
       write(uwrt,*)'IT = ',iteration,' nombres de lacunes ',nvac
       write(uwrt,*)'IT = ',iteration,'nombres d_interstitiels ',nint
       write(uwrt,*)'IT = ',iteration,'nombres d_antisites ',nanti
       write(uwrt,*)'IT = ',iteration,'nombres d_remplacments ',nremp

       if(lpdep)     write(uwrt,*)'IT = ',iteration,'nombres de remplacements ',nremp
       if (lpdef) then
          call  plt_extr(nvac,indvac,'vacancies',atc,boxc)
          call  plt_extr(nint,indint,'interstitials',atc,boxc)
          call  plt_extr(nas,indas,'antisites',atc,boxc)
...
!!$  end subroutine subc
  ...
  subroutine plt_extr(nprt,indprt,nameprt,atprt,boxprt,itapp,plmin,plmax)
    ...
    if (nprt.gt.0) then
       ...
       !          write(uwrt,*)'nameplt ',nameprt,' ',nameplt
  ...
  subroutine plotpart(atcf,plmin,plmax,atplt,boxplt)
    ...
    !    allocate(indplt(imm))
    !    plmin=plmin*1.0d-8 ; plmax=plmax*1.0d-8
    write(uwrt,'(A,3F7.2,A,3F7.2)')'portion affichee entre ', plmin*1.0d8 ,' et ',plmax*1.0d8  
    !    write(uwrt,*)'portion affichee entre ', plmin*1.0d8 ,' et ',plmax*1.0d8  
    nplt=0
    atcf%lgul=.false.
    do i=1,atcf%im     
