...
!             hold(i1,i2)=boxcg%at(i1,i2)
  ...
  subroutine test_conv(Nt,Ft,lover,Vt,Rt,lvm )
    ...
    integer::i,i1,i2,ip,ic,j
    real(double)::fpstopsig,sigt(3,3),volu,invvolu,trh(3,3),invtrh(3,3),fpt(3,3)
...
!       write(6,*)'VOLUME',volu,boxcg%volu
       invVolu = 1.d0/volu
       fpt(:,:)=MatMul( sigt(:,:) , invtrh(:,:) )*volu
       do i=1,3
          do j=1,3
             fpt(i,j)=abs(fpt(i,j))
          end do
       end do
       fpstopsig= maxval(fpt(:,:))
!       fpstopsig=(sigstop/unitP)*(boxcg%volu**0.6666666)
       write(unitgc,*)'fsigmax fpstopsig',fsigmax, fpstopsig
       write(unitgc,*)' sigmax sigstop',sigmax,sigstop
