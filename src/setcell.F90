module setcell
  ...
  USE arret_ndm_mod,only: arret_ndm
  USE gen_com_m, ONLY:ldemitab,nvat,pi,rang,lrctest,ltpcel,lspacendm,lperiod,unitwb,lwrtb,lbabar,lmasterb
...
contains
  ...
  subroutine setnox(boxsn,celsn,rum,noxr,noyr,nozr)
    ...
    if ((ipotentiel.ne.20).and.(izonr<2)) then
       if (lrctest) then
          write (unitw, *) 'STOP ; supprimer avec lrctest=.false. dans din'
          call arret_ndm
       endif
    end if
    if (lwrt) write (unitw, *) 'nox,noy,noz dans .din =', nox, noy, noz

    if (nox<=0.or.noy<=0.or.noz<=0) then
       ! détermination de nox noy noz qui ne sont pas donnes dans .din
       !
       if (lwrt)write (unitw, *) 'calcul de nox noy noz !!!'
       ! ==== MODIF CLOUET 2 ====================
       if (izonr<2) then
#ifdef PARA
          if ((nprocspace.gt.1).and.(lspaceNDM.eqv..true.)) then
             write(unitw,*)'trop petite boite pour para'
             call arret_ndm
          end if
#endif

          if (lwrt) then
             WRITE(unitw,'(a)') "Boite trop petite: le nombre de cellules est fixe a son minimum"
          endif
       endif
       ...
       if (lwrt) THEN
          write (unitw,'(a)') 'nox noy noz calcules a partir de ru'
          WRITE(unitw,'(2(a,g12.4),a,i0)') '  nox = Int( ', boxsn%nzl(1),'/',rum,') = ', nox
          WRITE(unitw,'(2(a,g12.4),a,i0)') '  noy = Int( ', boxsn%nzl(2),'/',rum,') = ', noy
          WRITE(unitw,'(2(a,g12.4),a,i0)') '  noz = Int( ', boxsn%nzl(3),'/',rum,') = ', noz
       END IF
    ...
    if (lwrt) write(unitw,'(A,3G15.7)') 'celsizes ',celsn%celsize(:)
  ...
  subroutine setcellconf(celscf,atcf,boxcf,rumax,lverbose)
    ...
    nvat=3*natperc
    natperc=max(int(2*natperc),20)    


    if (lwrt) &
         write(unitw,*) 'natperc im/noxyz', natperc, atcf%im_glob/celscf%noxyz
    ...
    celscf%atincel=0
    if (lwrt)  write(unitw,*)'ltabvois',atcf%ltabvois

    if (atcf%ltabvois) then
       rvois=atcf%rvois
       if (rumax>rvois) then
          write (unitw, *) rang,' rvois trop petit rvois rumax ', rvois, rumax
          call arret_ndm
       endif
       ...
       if (izonr2<1) then
          write (unitw, *) rang,'trop petite boite pour rvois !!!'
       ...
       if(lwrt)         write (unitw, *) 'Nvois= ', nvois,atcf%im_glob,nvperat,rvois,boxcf%volu,voluperat
       atcf%nvois=nvois
