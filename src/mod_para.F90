...
subroutine exchange_tabdensity_blocking_ordered(tabdensity, imm, num_at_glob, psc, im)
  ...
  unw=700+myidsp
  !====================================================
  ! Pass 1: neighbors with LOWER ranks
  !====================================================
  do nproc_voisin = 1, psc%nbr_proc_voisin
!     write(unw,*)'pass1.1 ',nproc_voisin ; flush(unw)
     procv = psc%proc_voisin(nproc_voisin)
     if (procv >= myidsp) cycle
 !    write(unw,*)'pass1.2 ',nproc_voisin; flush(unw)
     call exchange_one_neighbor_sendrecv_ordered( &
...
!  write(6,*)'Pcall1',rang
  ...
  !====================================================
  do nproc_voisin = 1, psc%nbr_proc_voisin
!     write(unw,*)'pass2.1 ',nproc_voisin
     procv = psc%proc_voisin(nproc_voisin)
     if (procv <= myidsp) cycle
!     write(unw,*)'pass2.2 ',nproc_voisin; flush(unw)
     call exchange_one_neighbor_sendrecv_ordered( &
          procv, tabdensity, imm, im, psc, &
          send_ids, send_val, recv_ids, recv_val,nproc_voisin)
...
subroutine exchange_one_neighbor_sendrecv_ordered( &
     procv, tabdensity, imm, im, psc, &
     send_ids, send_val, recv_ids, recv_val,iproc_voisin)

  use T_kind_param_m, only : double
  implicit none

  integer, intent(in) :: procv, imm, im,iproc_voisin
  type(para_space_config), intent(in) :: psc
  real(double) :: tabdensity(imm)
  ...
  !----------------------------------------
  nb_at_send = 0
  do ncell_front = 1, psc%nbr_cell_frontiere(iproc_voisin)
     koo = psc%cell_frontiere(iproc_voisin, ncell_front)
...
!  write(unw,*)'avant echange piloté parproc', myidsp,' vers/depuis inice voisin indice genéral',iproc_voisin,procv; flush(unw)
  !----------------------------------------
  ! Ordered blocking exchange
  !----------------------------------------
  if (myidsp < procv) then
     ! I SEND FIRST
!     write(unw,*)'proc', myidsp,' va envoyer à ',iproc_voisin,procv; flush(unw)
...
!     write(unw,*)'proc', myidsp,' A ENVOYE à/ va recevoir ',iproc_voisin,procv; flush(unw)
...
!     write(unw,*)'proc', myidsp,' a recu de ',iproc_voisin,procv; flush(unw)
  else
     ! I RECV FIRST
 !    write(unw,*)'proc', myidsp,' va recevoir de ',iproc_voisin,procv; flush(unw)
     call MPI_RECV(nb_at_recv, 1, MPI_INTEGER, procv, 3001, MPI_COMM_space, status, ierr)
     allocate(recv_ids(nb_at_recv))
     allocate(recv_val(nb_at_recv))
     call MPI_RECV(recv_ids, nb_at_recv, MPI_INTEGER, procv, 3002, MPI_COMM_space, status, ierr)
     call MPI_RECV(recv_val, nb_at_recv, NDM_MPI_REAL_DOUBLE, procv, 3003, MPI_COMM_space, status, ierr)
  !   write(unw,*)'proc', myidsp,' a recu de/va envoyé ',iproc_voisin,procv; flush(unw)
     call MPI_SEND(nb_at_send, 1, MPI_INTEGER, procv, 3001, MPI_COMM_space, ierr)
     call MPI_SEND(send_ids, nb_at_send, MPI_INTEGER, procv, 3002, MPI_COMM_space, ierr)
     call MPI_SEND(send_val, nb_at_send, NDM_MPI_REAL_DOUBLE, procv, 3003, MPI_COMM_space, ierr)
   !  write(unw,*)'proc', myidsp,' a envoyé à ',iproc_voisin,procv; flush(unw)
  end if
!  write(unw,*)'proc', myidsp,' fin des echanges avec ',iproc_voisin,procv; flush(unw)
...
!!$
  ...
  subroutine transfert_atomes_frontieres(psc) ! seulement maj_atomes_frt_ftm
  type(para_space_config)::psc
    ...
    logical, allocatable :: recv_buff_lgc(:,:)       ! buffer de reception des variables logical
    real(double), allocatable :: recv_buff_dbl(:,:)  ! buffer de reception des variables reelles
    integer::    pt_at_ftm ,ncell_front,koo
...
!!$ end if
    ...
    do nproc_voisin = 1, psc%nbr_proc_voisin
       ...
       do ncell_front = 1, psc%nbr_cell_frontiere(nproc_voisin)
          nb_at_send = nb_at_send + celmp%nato(psc%cell_frontiere(nproc_voisin,ncell_front))
       enddo
...
!!$       do ncell_front = 1, psc%nbr_cell_frontiere(nproc_voisin)
!!$          cellf = psc%cell_ftm(ncell_front)
!!$
...
!!$                ! On complete le buffer
       ...
       do i_at = 1, recv_nb_val
          ...
          !          imf = imf + 1
          !          imana = imana + 1
          atmp%lgul(pt_at_ftm)        = recv_buff_lgc(1,i_at)

          ! mise a jour des variables entieres
          atmp%ityp(pt_at_ftm)        = recv_buff_int(1,i_at)
          atmp%ielat(pt_at_ftm)       = recv_buff_int(2,i_at)
          !          iwmax(im)       = recv_buff_int(3,i_at)
          atmp%num_at_glob(pt_at_ftm) = recv_buff_int(3,i_at)
          select type (atmp)
          class is (atom_config_arps)
             atmp%mov(pt_at_ftm) = recv_buff_int(4,i_at)
          end select
          atmp%proc_at(pt_at_ftm)=rang
          ...
          atmp%xp(1,pt_at_ftm) = recv_buff_dbl(1,i_at) 
          atmp%xp(2,pt_at_ftm) = recv_buff_dbl(2,i_at) 
          atmp%xp(3,pt_at_ftm) = recv_buff_dbl(3,i_at) 
          atmp%fp(1,pt_at_ftm) = recv_buff_dbl(4,i_at) 
          atmp%fp(2,pt_at_ftm) = recv_buff_dbl(5,i_at) 
          atmp%fp(3,pt_at_ftm) = recv_buff_dbl(6,i_at) 


          select type (atmp)
          class is (atom_config_d)
             atmp%vp(1,pt_at_ftm) = recv_buff_dbl(7,i_at) 
             atmp%vp(2,pt_at_ftm) = recv_buff_dbl(8,i_at) 
             atmp%vp(3,pt_at_ftm) = recv_buff_dbl(9,i_at)
          end select
          ival=9
          select type (atmp)
          class is (atom_config_e)
...
!!$             atmp%vp(3,pt_at_ftm) = recv_buff_dbl(9,i_at)
             if (atmp%lxpp) then
                do ic3=1,3
                   ival =ival+1;atmp%xpp(ic3,pt_at_ftm)= recv_buff_dbl(ival,i_at)
                end do
             end if

             if (atmp%lprteat)then
                ival =ival+1
                atmp%eat(pt_at_ftm)=recv_buff_dbl(ival,i_at)
             end if
             if (atmp%lsigat)then
                do ic1=1,3
                   do ic2=1,3
                      ival =ival+1
                      atmp%sigat(ic1,ic2,pt_at_ftm)=recv_buff_dbl(ival,i_at)
                   end do
                end do
             end if
             if (atmp%llangevin) then
                do ic3=1,3
                   ival =ival+1;atmp%glangv(ic3,pt_at_ftm)= recv_buff_dbl(ival,i_at)
                end do
             end if
             if (atmp%lax) then
                do ic3=1,3
                   ival =ival+1;atmp%ax(ic3,pt_at_ftm)= recv_buff_dbl(ival,i_at)
                end do
             end if
          end select
          select type (atmp)
          class is (atom_config_arps)
             do ic3=1,3
                ival =ival+1; atmp%fpr(ic3,pt_at_ftm)= recv_buff_dbl(ival,i_at)
             end do
             if (allocated(atmp%rho)) then
                ival =ival+1; atmp%rho(pt_at_ftm)= recv_buff_dbl(ival,i_at)
             end if
             if (allocated(atmp%fpg)) then
                do ic3=1,3
                   ival =ival+1; atmp%fpg(ic3,pt_at_ftm)= recv_buff_dbl(ival,i_at)
                end do
             end if
  ...
  end subroutine transfert_atomes_frontieres
