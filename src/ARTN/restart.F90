...
! $Id: restart.f90 1534 2015-12-14 15:59:38Z mickael $
...
subroutine restart_states( istatus, ieventcurrent, iterations, iatp )
  ...
  if ( boundary == 'T' ) then
     read(FRESTART,*) cell(:,1), scala
     read(FRESTART,*) cell(:,2), scalaref
     read(FRESTART,*) cell(:,3)
  else
  ...
  do i = 1, NATOMS
     Atom(i) = type_name(typat(i))
                                      ...
                                      ! DEBUG
  if ( iproc == 0 ) then
     write(unit6P,*) 'BART: restart file'
     write(unit6P,*) 'BART: box: ', box
     write(unit6P,*) 'BART: central_atom ', central_atom
     write(unit6P,'(a,(1p,e17.10,0p))') ' BART: total energy: ', total_energy
     write(unit6P,'(a,(1p,e17.10,0p))') ' BART: ref   energy: ', ref_energy
     write(unit6P,'(a,3f20.8)') ' BART: xref(1): ', xref(1), yref(1), zref(1)
     write(unit6P,'(a,3f20.8)') ' BART: x(1)   : ', x(1), y(1), z(1)
     write(unit6P,'(a,3f20.8)') ' BART: x(NATOMS): ', x(NATOMS), y(NATOMS), z(NATOMS)

     if ( istatus == 2 .or. istatus == 4 ) then
     write(unit6P,*) 'BART: previous_norm : ', diis_norm_restart
     write(unit6P,*) 'BART: last line ',maxter_r,eigen_min_r,eigenvalue_r,nsteps_after_eigen_min_r
     end if
     write(unit6P,*) 'BART: Reading atomic input positions from RESTART file,'
     write(unit6P,*) 'BART: please, ignore the next message. '
  end if
...
subroutine save_state( istatus, iter, direction )
  ...
  if ( boundary == 'T' ) then
	 write(FRESTART,*) cell(:,1), scala
	 write(FRESTART,*) cell(:,2), scalaref
	 write(FRESTART,*) cell(:,3)
  else
