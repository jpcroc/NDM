...
function f_stuff_intel_is246206 {

  # to stop stuff sometimes
  envs --create_setenv="${HOME}/mld_setenv_intel.tmp" # get current bash environ status
  ...
  [ -z ${MLD_TYPE} ] && export MLD_TYPE=INTEL  # mandatory else risky
  [ ! "${MLD_TYPE}" == "INTEL" ] && in_red_pause "ERROR: bash environment is not clean, continue at your risks"
  ...
  if [ -z ${SETVARS_COMPLETED} ]; then
    ...
    in_red 'need launched setvars.sh oneAPI, set it for INTEL. compiler and mkl and itac only'
...
EOT
  ...
  [ -z ${I_MPI_ROOT} ] && in_red_pause 'problem: inexisting ${I_MPI_ROOT} from oneAPI mpi\nfix it.'

  export MLD_MPI_ROODIR=$(dirname ${I_MPI_ROOT})
  export MLD_MPI_INSDIR="${I_MPI_ROOT}"
  ...
  tmp=$(which mpiifort 2> /dev/null) && in_green 'mpiifort at '${tmp} || in_red_pause 'problem no reached mpiifort wrapper'
  ...
  unset MLD_NPROC # force it automatic make -j ???
  # export MLD_NPROC=10 # max
  f_compile_milady_intel # do cmake
  ...
  envs --create_diff_setenv="${HOME}/mld_setenv_intel.tmp" # get current bash environ status
...
[ "${USER}" == "wambeke" ] && f_stuff_intel_is246206   # do it for me
