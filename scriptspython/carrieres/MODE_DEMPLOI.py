...
5/utiliser carry6 pour recoller les commentaires 

6/ pour élimner les ligens impaires
sed 'n; d' < outT2.N2.csv >outT2.N2.I.csv
Ou bien simplement trier selon badge dans ecel


js2:
#!/bin/bash -f
sed -e '/>/s/>//' <outT2.1 > tmp1
sed -e '/</s/<//' <tmp1 > tmp2
sed -e '/\,/s/\,/\ /g' <tmp2> tmp3
