	...
	with open(INP, 'r') as inp:
		...
		for defect in PHR[0].defect:
			try:
				...
				exit()
			try:
				print("prefactor",defect.name,defect.prefactor)#doStuff(a.property)
			except:
				defect.prefactor=[]
				for q in defect.q:
...
def calcmu(Ph) :
	...
	while critere > 1E-15 and securite < 20:
...
def calcx(Ph,simple=False) :
	...
	print("FTOTmu0",Ftot,Ph.E0,Ph.mu[0],Ph.mu[1])
	st1=Ph.stoich()
	print("Dmu",Ph.mu[0],Ph.mu[0]-Ph.muS[0],Ph.mu[1]-Ph.muS[1],st0,st1)
...
for itemp in range(len(PHT[0].TEMP)):
	...
	muA0=PHT[0].mu[0]
	muB0=PHT[0].mu[1]
	mumin=PHT[0].mu[0]-0.2
	mumax=PHT[0].mu[0]+1.7
	...
	for muA in MULOOP:
		...
		Ftot=FreeE(PHT[0])
		with open(f"muA.{temp}.out", "a") as muout :
			muout.write(f"   {Ftot} {PHT[0].mu[0]} {PHT[0].mu[1]} {stch} \n")
