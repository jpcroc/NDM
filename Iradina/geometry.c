...
int InitSpecialGeometry(){
  ...
  if(result!=0){
    printf("Error reading NP structure file %s.\n",filename);
    return result;
  }
  ...
  if(print_level>=2){printf("Simulation volume half sizes are:\t%g\t%g\t%g nm\n",target_half_x,target_half_y,target_half_z);}
...
int NP_DataReader(char* ParName, char* ParValue){
  ...
  if(strcmp(ParName,"NP_diameter")==0){ /* The NP diameter */
    ...
    if(print_level>=1){printf("NP diameter:\t %g nm\n",NP_diameter);}
    if(print_level>=1){printf("NP material:\t %s\n",ListOfMaterials[0].Name);}
  }
  return 0;
}
