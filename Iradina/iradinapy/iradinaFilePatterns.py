...
"""
all file patterns of supposedly (sometimes) created by iradinaGui
...
###################
"launchIradina.sh" : r"""#!/bin/bash

# example of use:
# cd $HOME/IRADINAGUI_WORKDIR/iradinaSim_test
# ./launchIradina.sh

# this script is in launch directory of iradina
aDir=`dirname $0`

cd $aDir
echo "launch iradina.exe code in directory: "`pwd`
which iradina.exe    #PATH supposed set

if [ -f lock ]; then
    echo "lock file exist and iradina supposed working in directory."
    echo "delete file lock if you are sure."
    cat lock
    echo "ERROR on locked iradina directory"
    echo "########## END"
    exit 2
fi
...
then
    ...
    mv -f lock lock_old
    echo "ERROR on iradina"
    cat errors
    echo "iradina.exe finished"
    echo "########## END"
    exit 2
fi
i
...
# date @DATE@
# author iradinaGUI @WHOAMI@
# Generated by iradinaGUI, for user use
...
def filterHeaderDat(line):
  """
  from file .dat uranie/salome/paraview, extract (key, value) from line
  '#COLUMN_UNITS: cm | cm |...' -> ('COLUMN_UNITS', ['cm','cm',...])
  '#NAME: iris' -> ('NAME', 'iris')
  """
  sp = line.split(": ")
  if sp[0][0] != "#":
