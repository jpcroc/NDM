...
###################################################################
class IradinaSettings(SETT.Settings):
  """
  ...
  def __init__(self):
    super(IradinaSettings, self).__init__()
    self.name = "IradinaSettings"


#singleton setting for iradina
_Settings = IradinaSettings()
...
def getIradinaSysMacrosDir():
  rootdir = getExpandedVar("_IRADINAGUI_ROOT_DIR")
  if rootdir is None:
    # raise Exception("user have to set env var 'IRADINAGUI_ROOT_DIR'")
    logger.critical("user have to set env var 'IRADINAGUI_ROOT_DIR'")
    return None
  aDir = os.path.join(rootdir, "macros")
  if not os.path.isdir(aDir):
    logger.debug("inexisting ${IRADINAGUI_ROOT_DIR}/macros dir:\n  '%s'" % aDir)
    #devel uranie for gaudier, current dir
    aDir = os.getcwd()
  logger.debug("getIradinaSysMacrosDir %s" % aDir)
  return aDir

# policy: 
#   names var begins with "_" contains environment variable var reference
#   theses setting are evaluated LATER,
#   when using USET.getExpandedVar("_IRADINA_ROOT_DIR"), for example

#_Settings.setVar("_URANIESYS", "${URANIESYS}")
_Settings.setVar("_IRADINA_ROOT_DIR", "${IRADINA_ROOT_DIR}")
# _Settings.setVar("_ROOTSYS", "${ROOTSYS}")
#_Settings.setVar("_URANIEWORKDIR", "${URANIEWORKDIR}")
_Settings.setVar("_IRADINAGUI_WORKDIR", "${IRADINAGUI_WORKDIR}")
#_Settings.setVar("_URANIEGUIDIR", "${URANIEGUIDIR}")
_Settings.setVar("_IRADINAGUI_ROOT_DIR", "${IRADINAGUI_ROOT_DIR}")
...
#setEnvVarByDefault("${IRADINAGUI_WORKDIR}", os.getcwd())
