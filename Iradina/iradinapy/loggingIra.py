...
_verbose = False # True
_name = "loggingIra"
_loggerDefaultName = 'IraDefaultLogger'
_loggerUnittestName = 'IraUnittestLogger'
...
def getStrLogRecord(logRecord):
  ...
  """
  import iradinapy.coloringIra as COLS # needs sys.path.insert(0, iradir)
...
class LoggerIra(LOGI.Logger):
  ...
  def __init__(self, name, level=LOGI.INFO):
    ...
    """
    super(LoggerIra, self).__init__(name, level)
    ...
    self._loggerParamiko = None # set only if iradina jobs, useless for now
    self._fileParamiko = None # set when main command set iradina logger
  ...
  def setLoggerParamiko(self):
      ...
      which is defined here with an handler to automatic named .txt file.
      ...
      if self._loggerParamiko is None:
        ...
        if len(loggerPrmk.handlers) != 0:
          self.warning("logger paramiko have handler set yet ouside iradina, is a surprise")
  ...
  def getMainCommandHandler(self):
    ...
    for human user eye iradinaGUI outputs
    """
  ...
  def close(self):
    """
    final stuff for logger, done at end iradinaGui
    ...
    # todo etc
    self.isClosed = True # done at end iradinaGUI, flushed closed xml files.
    return
  ...
  def setFileHandlerForCommand(self, cmdParent, cmdInstance):
    ...
    |   etc.
    """
    import iradinapy.utilsIra as UTS # needs sys.path.insert(0, iradir)

    logger = self
    config = cmdInstance.getConfig()
    
    #import iradinapy.debug as DBG # avoid cross import
    ...
    if self.idCommandHandlers == 0: # first main command
      ...
      # same name for paramiko logs if needed, useful for iradineGUI logs only
      # but other JOBS directory  
      self._fileParamiko = os.path.join(log_dir_jobs, nameFileTxt) # useful for iradinaGUI logs only
  ...
  def initLinkForCommand(self, cmdParent, cmdNew):
    import iradinapy.linksXml as LIXML
...
#################################################################
class StreamHandlerIra(LOGI.StreamHandler):
    """
...
class XmlHandler(BufferingHandler):
  ...
  def close(self):
    ...
    if self.isClosed:
      ...
      log(msg, True) #avoid iradinaGUI logging message in logger close phase
    ...
    xmlFile = XMLMGR.XmlLogFile(targetFile, "iradinaGUIcommand")
...
#################################################################
# methods to define two LoggerIra instances in iradinaGui,
# no more need
#################################################################
def initLoggerAsDefault(logger, fmt=None, level=None):
  ...
  handler = StreamHandlerIra(sys.stdout) # Logging vers console
...
def _getDefaultLogger():
  ...
  LOGI.setLoggerClass(LoggerIra) # to get LoggerIra instance with trace etc.
...
if __name__ == "__main__":
  ...
  # get path to iradinaGui sources
  iradir = os.path.dirname(os.path.dirname(__file__))
  # Make the src & commands package accessible from all code
  sys.path.insert(0, iradir)
  testMain() 
  # here we have sys.exit()
else:
  # in production
  # get two LoggerIra instance used in iradinaGui, no more needed.
