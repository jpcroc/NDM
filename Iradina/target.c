...
int InitializeMaterials(char* Filename){
  ...
  */
  ...
  if(mem_usage_only==0){
    ...
    if(print_level>=0){printf("Prepare scattering matrices for ion on target collisions...");fflush(stdout);}
    for(i=0;i<MAX_ELEMENT_NO;i++){
    ...
    if(print_level>=0){printf("Prepare scattering matrices for recoil on target collisions...");fflush(stdout);}
...
int InitializeTargetStructure(char* Filename){
  ...
  if(mem_usage_only==0){
    ...
    if(result!=0){
      printf("Error: cannot read target composition file %s (error %i).\n",TargetCompositionFileName,result);
      return result;
    }
    if(print_level>=0){printf("Target composition read from %s.\n",TargetCompositionFileName);}
#ifdef DEBUG_MODE4
    ...
    for(i=0;i<NumberOfMaterials;i++){
      if(print_level>=1){printf("Material %i found in %i cells.\n",i,ListOfMaterials[i].CellCount);}
    }
    ...
    if(UseDensityMult==1){  /* If applicable, read the density multiplicators */
      ...
      if(result!=0){
	printf("Error: cannot read density multiplicator file %s (error %i).\n",TargetDensityMultFileName,result);
	return result;
      }
      if(print_level>=0){printf("Density multiplicators read from %s.n",TargetDensityMultFileName);}
    }
...
int TargetStructure_DataReader(char* ParName, char* ParValue){
  ...
  if(strcmp(ParName,"special_geometry")==0){ /* ... */
    ...
    if(print_level>0){printf("Special geometry in use: %i\n",special_geometry);}
  }
#endif
