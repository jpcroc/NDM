...
int IrradiateTarget(){
  ...
  /* init counters: */
  disp_c=0;miss_c=0;coll_c=0;sputter_c=0;ion_c=0;repl_c=0;leaving_ions_c=0;leaving_recoils_c=0;vac_c=0;int_c=0;escape_solid_c=0;replacer_escaped=0;recoil_counter=0;

  /*  subreplace_c=0; superthres_leave_c=0; subthres_created_c=0; subthres_reach_egde_c=0; superthres_created_counter_c=0; sub_become_super_c=0; */
  ...
  if(store_ion_paths==1){ /* if requested, store the exact paths of the ions */
    ...
    if(ion_paths_fp==NULL){
      printf("Error: Cannot open file for storing ion paths!\n");
      return -1;
    }
  }
  if(store_recoil_cascades==1){ /* if requested, store the exact paths of the ions */
    ...
    if(recoil_cascades_fp==NULL){
      printf("Error: Cannot open file for storing recoil cascades!\n");
      return -1;
    }
  }
  ...
  if(store_range3d>=1){ /* if requested, store the final positions of ions, similar to TRIMs range_3d.txt (only ions stopped inside target!) */
    ...
    if(store_range3d_fp==NULL){
      printf("Error: Cannot open range3d-file!\n");
      return -1002;
    }
  }
  ...
  for(i=0;i<max_no_ions;i++){ /* Create the ions and let them impinge on the target */
...
#ifdef DEBUG_MODE
    printf("DEBUG %s, l %i, ion %i\n",__FILE__,__LINE__,ion_c);fflush(stdout);
#endif
    /* Write status file */
    ...
    /* Display progress */
    if(i%display_interval==0){printf("Ions completed: %i\n",i);fflush(stdout);}
    ion_c++;
    ...
    if(((i%storage_interval)==0)&&(i>0)){ /* Intermediate storing */
      ...
      /*      if(print_level>-1){printf("Storing results after ion no. %i, %i displacements, %i replacements, %i missed collision...",i,disp_c,repl_c,miss_c);fflush(stdout);}*/
      if(print_level>-1){printf("Storing results after ion no. %i ...\n",i);fflush(stdout);}
    ...
    case 0: /* accurate */
      ...
      if(store_range3d==2){fprintf(store_range3dV_fp,"C %i\n",i);} 
      if(store_range3d==2){fprintf(store_range3dI_fp,"C %i\n",i);} 
    ...
    default: /* fast, like corteo */
      ...
      if(store_range3d==2){fprintf(store_range3dV_fp,"C %i\n",i);} 
      if(store_range3d==2){fprintf(store_range3dI_fp,"C %i\n",i);} 
      FastProjectileTransport(ionZ,ionM,ionInitialEnergy,x,y,z,ion_vx,ion_vy,ion_vz,1,0,0,0);
      break;
    }
...
int FastProjectileTransport(int ProjZ, float ProjM, double ProjE, float Proj_x, float Proj_y, float Proj_z, float Proj_vx, float Proj_vy, float Proj_vz, int is_ion, int OrgMaterial, int OrgElement, int OrgCell){
  /* Calculates the path of a projectile with given porperties (proton number, mass, energy, coordiantes, velocity unit vector (directional cosines) through the target material. Can be called recursively to follow recoils. The "is_ion" parameter must be 1 when an initial ion is handled or 0 when recoils are simulated, so the function will know where to store results and which scattering matrix is needed (shouldn't be necessary for the physics though). If the projectile is a recoil from within the target, the material and element and cell that it originated from must be known on order to store correctly the distribution of implanted recoils for each material*/
  ...
  */
  ...
  float e_disp,e_latt,e_surf=0;          /* Displacement, lattice and surface energy of target atoms */
  ...
  float xsi ,pmax,bmax ;/* 7.40-7.41 page 7-16 */
  /* init values */
  ...
  /*CROC: if from inside bulk cascade the initila site is a vacancy*/
  ...
  while(energy>0){ /* The projectile still has energy and can move on */
    ...
    /*old_cell=cell_i;*/ /* Store current stuff, before we change anything */ /* Not needed anymore in fast version of transport */
	...
	/* Flight length between collisions: Poisson-distributed value with interatomic spacing
	   as mean free path */
	...
	/* for each target material and element, store the number of atom ejected out of the simulation
	...
	if(impact_par >= temp1) {  
	  if(flight_length_type <3) {  
	  /* impact parameter larger than interatomic distance: assume collision has missed */
	  ...
	  /* after many collisions, the velocity starts to deviate from 1, because of the limited accuracy of floats.
	     We might therefore correct it */
	  ...
	  if(isnan(vx)) {
	    if(is_ion==1){
	      ...
	      printf("Rotation error occured (caused by a recoil in cascade from ion no. %i).\n",ion_c);
	    }
	    ...
	    printf("The projectile is discarded.\n");
	    return -100;
	  }
#endif
	  ...
	  if(simulation_type<3){ /* We need to consider the recoil */
	    ...
	    */
		...
		FastProjectileTransport(target_Z, target_mass, recoil_energy,
					...
					recoil_vx, recoil_vy, recoil_vz, 0,current_material_index,target_index,cell_i); 
		...
		FastProjectileTransport(target_Z, target_mass, recoil_energy, x, y, z,
	      ...
	      if(proj_eq_target==1){ /* this only happens if they are the same, otherwise projectile can
					only become an interstitial if stopped */
		/* Now check whether the projectile might replace the recoil */
		E_compare=e_latt;
		/*CROC !! E_compare set to e_latt ! CHANGE OF DEFAULT BEHAVIOUR !
		E_compare=e_disp;
		*/
	    ...
	    } else { /* Recoil cannot be displaced, because e<e_disp
...
int FullProjectileTransport(int ProjZ, float ProjM, double ProjE, float Proj_x, float Proj_y, float Proj_z, float Proj_vx, float Proj_vy, float Proj_vz, int is_ion, int OrgMaterial, int OrgElement, int OrgCell, char ProjState, int RD){
  ...
  */
  ...
  long int recoil_number;               /* Identifying unique number of this recoil */
  float E_surf, E_disp, E_latt;         /* Energy barriers: surface binding, perpendicular energy component,
					   displacement threshold and lattice binding energy */
  ...
  while(energy>0){ /* let projectile proceed until all its energy is lost */
    ...
    if(isvac==1){ /* vacuum */
      ...
      if(iranlist>=MAXRANLIST-2)iranlist = 0; /* reset list pointer of needed */
      stopping=0.0;                           /* no stopping or straggling in vacuum */
      straggling=0.0;
    } else { /* no vacuum */
      //      if((ion_c>MONITOR_ION)&&(1)){printf("---Density: %g\n", cell_c->DensityNM);}
...
#ifdef DEBUG_MODE
      if(ion_c>=MONITOR_ION){printf("DEBUG %s, line %i.\n",__FILE__,__LINE__);fflush(stdout);}
#endif
	...
	straggling*=(energy*0.0625);
      ...
      /* To Do: Idea: for very small energy, perhapse increase stopping to MAKE things stop quickly! */
      //      if(energy<(min_energy/2.0)){stopping=energy;straggling=0;}

    }

#ifdef DEBUG_MODE
    printf("DEBUG %s, line %i. Stop: %g, Stragg: %g\n",__FILE__,__LINE__,stopping,straggling);
#endif
    ...
    if(ion_left_target!=0){ /* projectile left sample! */
      ...
      /* Outside the simulation there should be vacuum...! If the last position was material,
	 we should make sure that the projectile has sufficient energy to overcome
...
#ifdef DEBUG_MODE
      if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i.\n",__FILE__,__LINE__);}
      if(ion_c>MONITOR_ION){  printf("proj xzy: %g %g %g\n",x,y,z);}
#endif
...
#ifdef DEBUG_MODE
      printf("DEBUG %s, l %i, new mat index: %i\n",__FILE__,__LINE__,new_material_index);fflush(stdout);
#endif
	...
	if(isvac==1){ /* old material is vacuum */
	  if(isvac2==0){   /* new material is not vacuum */
...
#ifdef DEBUG_MODE
	    if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i. E_Surf: %g\n",__FILE__,__LINE__,E_surf);}
#endif
...
#ifdef DEBUG_MODE
	      if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i. normal vec: %g %g %g.\n",__FILE__,__LINE__,nx,ny,nz);}
#endif
#ifdef INCLUDE_SPECIAL_GEOMETRY
	    } else {
	    ...
	    if(ion_c>MONITOR_ION){
	      printf("into mat: Normal vec: nx=%f, ny=%f, nz=%f\n",nx,ny,nz);
	      printf("into mat: Before refr: E=%f, vx=%f, vy=%f, vz=%f\n",energy,vx,vy,vz);
	      printf("DEBUG line %i, vx: %f\n",__LINE__,vx);
	    }
#endif
	    ...
	    if(ion_c>MONITOR_ION){
	      printf("DEBUG %s, line %i.\n",__FILE__,__LINE__);
	      printf("into mat: After  refr: E=%f, vx=%f, vy=%f, vz=%f\n\n",energy,vx,vy,vz);
	    }
#endif
...
//	      max_impact_par = current_material->MeanImpactPar * sqrtloglist1[iranloglist];
	      ...
	      if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i, meip: %g.\n",__FILE__,__LINE__,new_material->MeanImpactPar);}
#endif
	      if(++iranloglist>=MAXLOGLIST) iranloglist=0;
	      break;
	    case 1: /* atomic spacing */
...
#ifdef DEBUG_MODE
	  if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i, ip: %g, mip: %g.\n",__FILE__,__LINE__,impact_par,max_impact_par);}
#endif
...
#ifdef DEBUG_MODE
	  if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i. t_vec: %g %g %g.\n",__FILE__,__LINE__,target_x,target_y,target_z);}
#endif
	  ...
	  if(target_outside_simulation==0){ /* target nucleus is within simulation volume */
...
#endif
#ifdef DEBUG_MODE
	      if(ion_c>MONITOR_ION){  printf("DEBUG %s, line %i. TC: %i, t_vec: %g %g %g.\n",__FILE__,__LINE__,target_cell,target_x,target_y,target_z);}
#endif
		...
		ScatMatrix=&(scattering_matrices[ProjZ][targetZ]);
		 ...
		 fflush(stdout);
	      ...
	      if(impact_par >= temp1) {  /* impact parameter much larger than interatomic distance:
		...
		//		if(ion_c>=DEBUGIONLIMIT){printf("DE TR, l %i i%i\n",__LINE__,ion_c);fflush(stdout);}
		  ...
		  if(next_ProjState==0){ /* really displaced! (Energy was larger than displacement energy) */
		    ...
		    disp_c++;vac_c++;
		    /* check if projectile might replace the recoil. Condition for replacement collision:
		       1. equal particles, 2. proj has less than disp energy, 3. proj was not marked sub-threshold, 4. recoil is not sub-threshold */
		    if( (proj_eq_target==1)){

		      if(energy<E_disp){
		/*CROC!!
		      if(energy<E_latt)! CHANGE OF DEFAULT BEHAVIOUR !{
		*/
			if((ProjState&1)==0){
	...
	if(isvac2==0){   /* no vacuum, stop, because of too little energy */
	  if(is_ion==1){                  /* its an ion that has to stop */
	    ...
	    if( (ProjState&1)==0){        /* projectile was not sub-threshold, so it was really displaced. It's allowed to becomes an interstitial */
